...
final class CrawlRelaxSanProductDataCommand extends Command
{
    protected $signature = 'relaxsan:crawl-product-data
        {--from=scrapers/relaxsan/product-links.json : Product link discovery JSON file under storage/app.}
        {--url=* : Explicit RelaxSan product URL to scrape instead of reading --from.}
        ...
        {--show-failures : Print failed RelaxSan product URLs.}';

    protected $description = 'Scrape RelaxSan product details from discovered product URLs into one JSON dataset.';

    public function __construct(
        private readonly RelaxSanProductDataCrawler $crawler,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($explicitUrls !== []) {
            if (! $json) {
                $this->info('Scraping RelaxSan product data from explicit product URLs...');
            }
            ...
            if (! $json) {
                $this->info('Scraping RelaxSan product data from saved product-link discovery JSON...');
            }
        ...
        if ((bool) $this->option('show-failures') && $result['failed_urls'] !== []) {
            $this->newLine();
            $this->warn('Failed RelaxSan product URLs:');
