...
final class CrawlAntarProductDataCommand extends Command
{
    protected $signature = 'antar:crawl-product-data
        {--from=scrapers/antar/product-links.json : Product link discovery JSON file under storage/app.}
        {--url=* : Explicit Antar product URL to scrape instead of reading --from.}
        ...
        {--attempts=3 : Number of attempts per Antar product request.}
        {--retry-delay-ms=1000 : Milliseconds to pause before retrying a failed Antar product request.}
        ...
        {--show-failures : Print failed Antar product URLs.}';

    protected $description = 'Scrape Antar product details from discovered product URLs into one JSON dataset.';

    public function __construct(
        private readonly AntarProductDataCrawler $crawler,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($explicitUrls !== []) {
            if (! $json) {
                $this->info('Scraping Antar product data from explicit product URLs...');
            }
            ...
            if (! $json) {
                $this->info('Scraping Antar product data from saved product-link discovery JSON...');
            }
        ...
        if ((bool) $this->option('show-failures') && $result['failed_urls'] !== []) {
            $this->newLine();
            $this->warn('Failed Antar product URLs:');
