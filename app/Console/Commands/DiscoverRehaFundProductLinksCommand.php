...
final class DiscoverRehaFundProductLinksCommand extends Command
{
    protected $signature = 'rehafund:product-links
        {--url=* : RehaFund category/menu page to scan. Defaults to https://sklep.rehafund.pl/.}
        {--categories-from= : Use an existing category discovery JSON file under storage/app instead of discovering categories again.}
        {--category-url=* : Explicit RehaFund category URL to scrape. When used, hierarchy discovery is skipped.}
        ...
        {--request-delay-ms=500 : Milliseconds to pause before each RehaFund HTTP request.}
        ...
        {--show-failures : Print failed RehaFund URLs.}';

    protected $description = 'Discover RehaFund product URLs from product-scraping categories.';

    public function __construct(
        private readonly RehaFundProductUrlScraper $scraper,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($categoryUrls !== []) {
            if (! $json) {
                $this->info('Discovering RehaFund product URLs from explicit category URLs...');
            }
            ...
            if (! $json) {
                $this->info('Discovering RehaFund product URLs from saved category discovery JSON...');
            }
            ...
            if (! $json) {
                $this->info('Discovering RehaFund product URLs from category hierarchy...');
            }

            $urls = $this->option('url') ?: [RehaFundCategoryUrlScraper::DEFAULT_URL];
        ...
        if ((bool) $this->option('show-failures') && ($result['failed_urls'] ?? []) !== []) {
            $this->newLine();
            $this->warn('Failed RehaFund URLs:');
