...
final class CrawlVayaProductDataCommand extends Command
{
    protected $signature = 'vaya:crawl-product-data
        {--from=scrapers/vaya/product-links.json : Product link discovery JSON file under storage/app.}
        {--url=* : Explicit Vaya product URL to scrape instead of reading --from.}
        ...
        {--retry-delay-ms=1500 : Milliseconds to pause before retrying a Vaya request.}
        ...
        {--show-failures : Print failed Vaya product URLs.}';

    protected $description = 'Scrape Vaya product details from discovered product URLs into one JSON dataset.';

    public function __construct(
        private readonly VayaProductDataCrawler $crawler,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($explicitUrls !== []) {
            if (! $json) {
                $this->info('Scraping Vaya product data from explicit product URLs...');
            }
            ...
            if (! $json) {
                $this->info('Scraping Vaya product data from saved product-link discovery JSON...');
            }
        ...
        if ((bool) $this->option('show-failures') && $result['failed_urls'] !== []) {
            $this->newLine();
            $this->warn('Failed Vaya product URLs:');
    ...
    private function saveJson(string $relativePath, array $result, bool $quiet = false): void
    {
        ...
        if (! is_dir($directory) && ! mkdir($directory, 0755, true) && ! is_dir($directory)) {
            throw new RuntimeException('Unable to create Vaya product-data directory: '.$directory);
        }
        ...
        if (file_put_contents($path, $json) === false) {
            throw new RuntimeException('Unable to save Vaya product data JSON: '.$path);
        }
