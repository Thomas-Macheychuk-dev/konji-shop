...
final class CrawlNovicareProductDataCommand extends Command
{
    protected $signature = 'novicare:crawl-product-data
        {--from=scrapers/novicare/product-links.json : Product link discovery JSON file under storage/app.}
        {--url=* : Explicit Novicare product URL to scrape instead of reading --from.}
        ...
        {--retry-delay-ms=1500 : Milliseconds to pause before retrying a Novicare request.}
        ...
        {--show-failures : Print failed Novicare product URLs.}';

    protected $description = 'Scrape Novicare product details from discovered product URLs into one JSON dataset.';
    ...
    public function handle(): int
    {
        ...
        if ($explicitUrls !== []) {
            if (! $json) {
                $this->info('Scraping Novicare product data from explicit product URLs...');
            }
            ...
            if (! $json) {
                $this->info('Scraping Novicare product data from saved product-link discovery JSON...');
            }
        ...
        if ((bool) $this->option('show-failures') && $result['failed_urls'] !== []) {
            $this->newLine();
            $this->warn('Failed Novicare product URLs:');
    ...
    private function printProductSummary(array $result): void
    {
        ...
        foreach ($result['products'] as $product) {
            ...
            $this->line('  Product code: '.($product['product_code'] ?? 'not found'));
            $this->line('  Category: '.($product['category'] ?? 'not found'));
            $this->line('  Sizes: '.count($product['variant_candidates'] ?? []));
            $this->line('  Indications: '.count($product['indications'] ?? []));
            $this->line('  Images: '.count($product['images']));
            $this->line('  Attributes: '.count($product['attributes']));
            $this->line('  Related products: '.count($product['related_products'] ?? []));
        }
    }
    ...
    private function saveJson(string $relativePath, array $result, bool $quiet = false): void
    {
        ...
        if (! is_dir($directory) && ! mkdir($directory, 0755, true) && ! is_dir($directory)) {
            throw new RuntimeException('Unable to create Novicare product-data directory: '.$directory);
        }
        ...
        if (file_put_contents($path, $json) === false) {
            throw new RuntimeException('Unable to save Novicare product data JSON: '.$path);
        }
