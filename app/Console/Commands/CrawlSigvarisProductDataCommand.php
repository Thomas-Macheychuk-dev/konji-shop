...
final class CrawlSigvarisProductDataCommand extends Command
{
    ...
    public function __construct(private readonly SigvarisProductDataCrawler $crawler)
    {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if (! $json && ! (bool) $this->option('no-progress')) {
            $this->crawler->withProgressCallback(fn (string $m): null => $this->line($m));
        }
        ...
        if ($json) {
            ...
            foreach ($result['products'] ?? [] as $product) {
                $this->line('- '.($product['name'] ?? 'Unnamed').' | ID='.($product['external_product_id'] ?? '?').' | combination='.($product['default_combination_id'] ?? 'none').' | gross='.($product['price_gross_amount'] ?? '?').' PLN | images='.count($product['images'] ?? []).' | attributes='.count($product['attributes'] ?? []));
            }
        }
    ...
    private function limit(): ?int
    {
        ...
        return $v === null || $v === '' ? null : max(1, (int) $v);
    }
    ...
    private function loadJson(string $relative): array
    {
        ...
        if (! is_file($path)) {
            throw new JsonException('Sigvaris product-link JSON does not exist: '.$path);
    ...
    private function save(array $result, bool $quiet): void
    {
        ...
        if (! is_dir(dirname($path)) && ! mkdir(dirname($path), 0755, true) && ! is_dir(dirname($path))) {
            ...
            throw new RuntimeException('Unable to save Sigvaris product data.');
        }if (! $quiet) {
            $this->info('Saved product data to storage/app/'.ltrim($relative, '/'));
        }
    }
