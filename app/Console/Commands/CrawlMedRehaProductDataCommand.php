...
final class CrawlMedRehaProductDataCommand extends Command
{
    protected $signature = 'medreha:crawl-product-data
        {--from=scrapers/medreha/product-links.json : Product link discovery JSON file under storage/app.}
        {--url=* : Explicit MedReha product URL to scrape instead of reading --from.}
        ...
        {--show-failures : Print failed MedReha product URLs.}';

    protected $description = 'Scrape MedReha product details from discovered product URLs into one JSON dataset.';

    public function __construct(
        private readonly MedRehaProductDataCrawler $crawler,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($explicitUrls !== []) {
            if (! $json) {
                $this->info('Scraping MedReha product data from explicit product URLs...');
            }
            ...
            if (! $json) {
                $this->info('Scraping MedReha product data from saved product-link discovery JSON...');
            }
        ...
        if ((bool) $this->option('show-failures') && $result['failed_urls'] !== []) {
            $this->newLine();
            $this->warn('Failed MedReha product URLs:');
