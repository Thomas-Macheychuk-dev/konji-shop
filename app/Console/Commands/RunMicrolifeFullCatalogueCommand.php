...
final class RunMicrolifeFullCatalogueCommand extends Command
{
    protected $signature = 'microlife:full-catalogue
        {--from=scrapers/microlife/product-links.json : Product-link discovery JSON path under storage/app.}
        {--runtime-dir=scrapers/microlife/runtime : Runtime manifest and product-data batch directory under storage/app.}
        {--categories-from=scrapers/microlife/categories.json : Category discovery JSON path refreshed by --refresh-discovery.}
        {--refresh-discovery : Discover live Microlife categories and product links before starting a new runtime.}
        {--batch-size=5 : Number of Microlife product URLs to crawl and import per batch.}
        ...
        {--no-documents : Do not download or localize Microlife PDF/ZIP documents.}
        ...
        {--asset-retry-delay-ms=5000 : Milliseconds to wait between Microlife asset retries.}
        ...
        {--attempts=3 : Number of attempts per Microlife product request.}
        {--retry-delay-ms=1500 : Milliseconds to pause before retrying a failed Microlife product request.}
        {--request-delay-ms=500 : Milliseconds to pause before each Microlife product request.}
        {--insecure : Disable TLS certificate verification for Microlife crawl and asset requests.}
    ...
    protected $description = 'Run the Microlife full catalogue crawl and import in resumable, logged batches.';

    public function __construct(
        private readonly MicrolifeCategoryUrlScraper $categoryUrlScraper,
        private readonly MicrolifeProductDataCrawler $crawler,
        private readonly MicrolifeProductImporter $importer,
        private readonly MicrolifeProductUrlScraper $productUrlScraper,
    ) {
        parent::__construct();
    }
    ...
    {
        $sourceRelativePath = $this->relativePathOption('from', 'scrapers/microlife/product-links.json');
        $runtimeRelativePath = $this->relativePathOption('runtime-dir', 'scrapers/microlife/runtime');
        ...
        $refreshDiscovery = (bool) $this->option('refresh-discovery');

        if ($resume && $reset) {
            throw new InvalidArgumentException('--resume and --reset cannot be used together.');
        }

        if ($resume && $refreshDiscovery) {
            throw new InvalidArgumentException('--refresh-discovery cannot be used with --resume.');
        }
        ...
        if ($refreshDiscovery) {
            $this->refreshProductLinkDiscovery($sourcePath, $continueOnFailure);
        }

        $source = $this->loadJson($sourcePath, 'Microlife product-link discovery JSON');
        $sourceContents = file_get_contents($sourcePath);

        if ($sourceContents === false) {
            throw new RuntimeException('Unable to read Microlife product-link discovery JSON: '.$sourcePath);
        }

        $productUrls = $this->productUrls($source);

        if ($productUrls === []) {
            $this->error('No Microlife product URLs were found in storage/app/'.$sourceRelativePath);
        ...
        $importDocuments = ! $dryRun && ! $crawlOnly && ! (bool) $this->option('no-documents');
        ...
        if ($resume) {
            ...
            $manifest = $this->loadJson($manifestPath, 'Microlife runtime manifest');
            ...
            $this->error('A Microlife runtime manifest already exists. Use --resume or --reset.');
        ...
        $this->info('Running Microlife full catalogue runtime...');
        ...
        $this->line('Discovery refreshed: '.($refreshDiscovery ? 'yes' : 'no'));
        ...
        while ($nextOffset < count($productUrls)) {
            ...
            if ($reuseBatchData) {
                ...
                $productData = $this->loadJson($batchPath, 'Microlife product-data batch JSON');
            } else {
            ...
            if ($crawlFailures !== [] || $crawlStoppedEarly) {
                ...
                $batch['error'] = $crawlStoppedEarly
                    ? (string) ($productData['stop_reason'] ?? 'Microlife product crawl stopped early')
                    : 'One or more Microlife product URLs failed to crawl.';
                $manifest['batches'][$batchKey] = $batch;
            ...
            $manifest['counts']['crawl_failures'] = $this->sumBatchField($manifest, 'crawl_failure_count');
            $manifest['counts']['import_failures'] = $this->sumBatchField($manifest, 'import_failure_count');
            $manifest['counts']['import_warnings'] = $this->sumBatchField($manifest, 'import_warning_count');
            $hadFailures = $manifest['counts']['crawl_failures'] > 0
                || $manifest['counts']['import_failures'] > 0
                || $manifest['counts']['import_warnings'] > 0;
            ...
            $this->info(sprintf(
                'Batch complete: crawled %d, imported %d, crawl failures %d, import failures %d, import warnings %d',
                $batch['product_count'],
                $batch['imported_count'],
                $batch['crawl_failure_count'],
                $batch['import_failure_count'],
                $batch['import_warning_count'],
            ));
        ...
        if (! $dryRun && ! $crawlOnly) {
            $audit = $this->auditRuntime($manifest, $runtimeRelativePath, $imageLimit, $importImages);
        ...
        $this->newLine();
        $this->info('Microlife full catalogue runtime finished with status: '.$manifest['status']);
        ...
        $this->line('Import warnings: '.$manifest['counts']['import_warnings']);

        if (is_array($manifest['audit'] ?? null)) {
            $audit = $manifest['audit'];
            $this->line('Audit products: '.$audit['database_products'].'/'.$audit['expected_products']);
            $this->line('Audit variants: '.$audit['database_variants'].'/'.$audit['expected_variants']);
            $this->line('Audit images: '.$audit['database_images'].'/'.$audit['expected_images']);
            $this->line('Audit products without variants: '.$audit['products_without_variants']);
            $this->line('Audit products without images: '.$audit['products_without_images']);
            $this->line('Audit products without categories: '.$audit['products_without_categories']);
            $this->line('Audit duplicate external IDs: '.$audit['duplicate_external_ids']);
            $this->line('Audit duplicate variant identities: '.$audit['duplicate_variant_identities']);
            $this->line('Audit report: '.$audit['report_path']);
        }
    ...
    private function auditRuntime(
        ...
        foreach (($manifest['batches'] ?? []) as $batch) {
            ...
            $storedPath = (string) ($batch['product_data_path'] ?? '');

            if (! str_starts_with($storedPath, 'storage/app/')) {
                continue;
            }

            $batchPath = storage_path('app/'.substr($storedPath, strlen('storage/app/')));
            ...
            foreach (($productData['products'] ?? []) as $product) {
                ...
                $variantCount = count(is_array($product['variant_candidates'] ?? null)
                    ? $product['variant_candidates']
                    : []);
                $expectedVariants += max(1, $variantCount);
        ...
        $databaseVariants = (int) $products->sum('variants_count');
        $databaseImages = (int) $products->sum('images_count');
        $productsWithoutVariants = $products->where('variants_count', 0)->count();
        $productsWithoutImages = $expectImages ? $products->where('images_count', 0)->count() : 0;
        $productsWithoutCategories = $products->where('categories_count', 0)->count();
        $duplicateExternalIds = DB::table('products')
            ->where('external_source', 'microlife')
            ->select('external_id')
            ->groupBy('external_id')
            ->havingRaw('COUNT(*) > 1')
            ->get()
            ->count();
        $duplicateVariantIdentities = 0;

        if (Schema::hasColumn('product_variants', 'external_variant_id')) {
            $duplicateVariantIdentities = DB::table('product_variants')
                ->whereIn('product_id', $products->pluck('id'))
                ->whereNotNull('external_variant_id')
                ->where('external_variant_id', '!=', '')
                ->select(['product_id', 'external_variant_id'])
                ->groupBy(['product_id', 'external_variant_id'])
                ->havingRaw('COUNT(*) > 1')
                ->get()
                ->count();
        }

        $passed = $products->count() === count($ids)
            && $databaseVariants >= $expectedVariants
            && $productsWithoutVariants === 0
            && $productsWithoutCategories === 0
            && $duplicateExternalIds === 0
            && $duplicateVariantIdentities === 0
            && (! $expectImages || ($databaseImages >= $expectedImages && $productsWithoutImages === 0));
        $reportRelativePath = $runtimeRelativePath.'/audit.json';
        $reportPath = storage_path('app/'.$reportRelativePath);
        $audit = [
            'passed' => $passed,
            'expected_products' => count($ids),
            'database_products' => $products->count(),
            'expected_variants' => $expectedVariants,
            'database_variants' => $databaseVariants,
            ...
            'duplicate_variant_identities' => $duplicateVariantIdentities,
            'report_path' => 'storage/app/'.$reportRelativePath,
            'generated_at' => now()->toIso8601String(),
        ];

        $this->saveJson($reportPath, $audit);

        return $audit;
    }

    private function refreshProductLinkDiscovery(string $sourcePath, bool $continueOnFailure): void
    {
        $categoriesRelativePath = $this->relativePathOption(
            'categories-from',
            'scrapers/microlife/categories.json',
        );
        $categoriesPath = storage_path('app/'.$categoriesRelativePath);
        $verifyTls = ! (bool) $this->option('insecure');
        $timeout = $this->positiveIntOption('timeout', 20);
        $attempts = $this->positiveIntOption('attempts', 3);
        $retryDelayMs = $this->nonNegativeIntOption('retry-delay-ms', 1500);
        $requestDelayMs = $this->nonNegativeIntOption('request-delay-ms', 500);

        $this->info('Refreshing Microlife category discovery...');

        $categories = $this->categoryUrlScraper
            ->withTlsVerification($verifyTls)
            ->withTimeout($timeout)
            ->withAttempts($attempts)
            ->withRetryDelayMilliseconds($retryDelayMs)
            ->withRequestDelayMilliseconds($requestDelayMs)
            ->withProgressCallback(fn (string $message): null => $this->line('  '.$message))
            ->scrape();

        $this->saveJson($categoriesPath, $categories);
        $this->line('Discovered categories: '.count($categories['categories'] ?? []));
        $this->line('Product categories: '.count($categories['product_category_urls'] ?? []));
        $this->line('Category discovery failures: '.count($categories['failed_urls'] ?? []));

        $this->info('Refreshing Microlife product-link discovery...');

        $result = $this->productUrlScraper
            ->withTlsVerification($verifyTls)
            ->withTimeout($timeout)
            ->withAttempts($attempts)
            ->withRetryDelayMilliseconds($retryDelayMs)
            ->withRequestDelayMilliseconds($requestDelayMs)
            ->withProgressCallback(fn (string $message): null => $this->line('  '.$message))
            ->scrapeFromDiscoveredCategories($categories);

        $this->saveJson($sourcePath, $result);
        $this->line('Discovered product URLs: '.count($result['product_urls'] ?? []));
        $this->line('Product-link discovery failures: '.count($result['failed_urls'] ?? []));

        $failures = array_merge(
            $this->stringMap($categories['failed_urls'] ?? []),
            $this->stringMap($result['failed_urls'] ?? []),
        );
        ...
        if ((bool) $this->option('show-failures')) {
            foreach ($failures as $url => $reason) {
                $this->line('- discovery '.$url.' — '.$reason);
            }
        }

        if (! $continueOnFailure) {
            throw new RuntimeException(
                'Microlife discovery reported failures. Retry or use --continue-on-failure.',
            );
        }
    }
    ...
    private function newManifest(
        ...
        return [
            ...
            'log_path' => 'storage/logs/microlife/full-catalogue-'.$runId.'.jsonl',
    ...
    private function validateResumeManifest(
        ...
        if (($manifest['source'] ?? null) !== 'microlife') {
            throw new RuntimeException('Runtime manifest is not a Microlife manifest.');
        }

        if (($manifest['source_sha256'] ?? null) !== $sourceHash) {
            throw new RuntimeException('Microlife product-link discovery JSON changed since this runtime started. Use --reset to start a new runtime.');
        }
        ...
        if ((int) ($manifest['total_product_urls'] ?? -1) !== $totalProductUrls) {
            throw new RuntimeException('Microlife product URL count changed since this runtime started. Use --reset to start a new runtime.');
        }

        if (($manifest['execution_configuration'] ?? null) !== $executionConfiguration) {
            throw new RuntimeException('Microlife runtime mode, status, or asset settings changed. Resume with the original settings or use --reset.');
        }
    }
    ...
    private function appendLog(array $manifest, string $event, array $context = []): void
    {
        ...
        if (! str_starts_with($logPath, 'storage/logs/')) {
            throw new RuntimeException('Invalid Microlife runtime log path.');
        }
        ...
        if (file_put_contents($path, $line.PHP_EOL, FILE_APPEND | LOCK_EX) === false) {
            throw new RuntimeException('Unable to append Microlife runtime operational log: '.$path);
        }
    }
    ...
    private function saveJson(string $path, array $data): void
    {
        ...
        if (file_put_contents($temporaryPath, $json, LOCK_EX) === false) {
            throw new RuntimeException('Unable to write temporary Microlife runtime JSON: '.$temporaryPath);
        }

        if (! rename($temporaryPath, $path)) {
            @unlink($temporaryPath);
            throw new RuntimeException('Unable to publish Microlife runtime JSON: '.$path);
        }
    }
    ...
    private function ensureDirectory(string $path): void
    {
        if (! is_dir($path) && ! mkdir($path, 0755, true) && ! is_dir($path)) {
            throw new RuntimeException('Unable to create Microlife runtime directory: '.$path);
        }
    }
    ...
    private function maybePrintFailures(array $manifest): void
    {
        ...
        foreach (($manifest['batches'] ?? []) as $batch) {
            ...
            foreach (($batch['crawl_failures'] ?? []) as $url => $reason) {
                if (! $printedHeading) {
                    $this->newLine();
                    $this->warn('Microlife runtime failures:');
                    $printedHeading = true;
                }
