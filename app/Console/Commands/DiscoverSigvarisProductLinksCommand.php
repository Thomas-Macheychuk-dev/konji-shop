...
final class DiscoverSigvarisProductLinksCommand extends Command
{
    ...
    public function __construct(private readonly SigvarisProductUrlScraper $scraper)
    {
        parent::__construct();
    }
    ...
    private function nullablePositive(string $name): ?int
    {
        $v = $this->option($name);
    ...
    private function loadJson(string $relative): array
    {
        ...
        if (! is_file($path)) {
            throw new JsonException('Sigvaris category JSON does not exist: '.$path);
        }
        $decoded = json_decode((string) file_get_contents($path), true, 512, JSON_THROW_ON_ERROR);
        if (! is_array($decoded)) {
            throw new JsonException('Sigvaris category JSON must contain an object.');
        }
    ...
    private function save(array $result, bool $quiet): void
    {
        ...
        if (! is_dir(dirname($path)) && ! mkdir(dirname($path), 0755, true) && ! is_dir(dirname($path))) {
            ...
            throw new RuntimeException('Unable to save Sigvaris product links.');
        } if (! $quiet) {
            $this->info('Saved product links to storage/app/'.ltrim($relative, '/'));
        }
    }
