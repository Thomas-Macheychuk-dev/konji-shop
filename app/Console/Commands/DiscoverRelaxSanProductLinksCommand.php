...
final class DiscoverRelaxSanProductLinksCommand extends Command
{
    protected $signature = 'relaxsan:product-links
        ...
        {--category-url=* : Explicit RelaxSan category URL to scrape. When used, hierarchy discovery is skipped.}
    ...
    protected $description = 'Discover RelaxSan product URLs from product-scraping categories.';

    public function __construct(
        private readonly RelaxSanProductUrlScraper $scraper,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($categoryUrls !== []) {
            if (! $json) {
                $this->info('Discovering RelaxSan product URLs from explicit category URLs...');
            }
            ...
            if (! $json) {
                $this->info('Discovering RelaxSan product URLs from saved category discovery JSON...');
            }
            ...
            if (! $json) {
                $this->info('Discovering RelaxSan product URLs from category hierarchy...');
            }
