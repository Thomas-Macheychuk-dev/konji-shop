...
final class DiscoverMedRehaProductLinksCommand extends Command
{
    protected $signature = 'medreha:product-links
        {--url=* : MedReha category/menu page to scan. Defaults to https://sklep.medreha.pl/pl/c/.}
        {--categories-from= : Use an existing category discovery JSON file under storage/app instead of discovering categories again.}
        {--category-url=* : Explicit MedReha category URL to scrape. When used, hierarchy discovery is skipped.}
        ...
        {--show-failures : Print failed MedReha URLs.}';

    protected $description = 'Discover MedReha product URLs from product-scraping categories.';

    public function __construct(
        private readonly MedRehaProductUrlScraper $scraper,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($categoryUrls !== []) {
            if (! $json) {
                $this->info('Discovering MedReha product URLs from explicit category URLs...');
            }
            ...
            if (! $json) {
                $this->info('Discovering MedReha product URLs from saved category discovery JSON...');
            }
            ...
            if (! $json) {
                $this->info('Discovering MedReha product URLs from category hierarchy...');
            }

            $urls = $this->option('url') ?: [MedRehaCategoryUrlScraper::DEFAULT_URL];
