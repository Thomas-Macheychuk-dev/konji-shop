...
final class CrawlReh4MatProductDataCommand extends Command
{
    protected $signature = 'reh4mat:crawl-product-data
        {--from=scrapers/reh4mat/product-links.json : Product link discovery JSON file under storage/app.}
        {--url=* : Explicit Reh4Mat product URL to scrape instead of reading --from.}
        ...
        {--show-failures : Print failed Reh4Mat product URLs.}';

    protected $description = 'Scrape Reh4Mat product details from discovered product URLs into one JSON dataset.';

    public function __construct(
        private readonly Reh4MatProductDataCrawler $crawler,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($explicitUrls !== []) {
            if (! $json) {
                $this->info('Scraping Reh4Mat product data from explicit product URLs...');
            }
            ...
            if (! $json) {
                $this->info('Scraping Reh4Mat product data from saved product-link discovery JSON...');
            }
        ...
        if ((bool) $this->option('show-failures') && $result['failed_urls'] !== []) {
            $this->newLine();
            $this->warn('Failed Reh4Mat product URLs:');
    ...
    private function printProductSummary(array $result): void
    {
        ...
        foreach ($result['products'] as $product) {
            ...
            $this->line('  Pictograms: '.count($product['pictograms']));
