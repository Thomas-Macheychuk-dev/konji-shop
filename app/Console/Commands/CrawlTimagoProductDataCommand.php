...
final class CrawlTimagoProductDataCommand extends Command
{
    protected $signature = 'timago:crawl-product-data
        {--from=scrapers/timago/product-links.json : Product link discovery JSON file under storage/app.}
        {--url=* : Explicit Timago product URL to scrape instead of reading --from.}
        ...
        {--show-failures : Print failed Timago product URLs.}';

    protected $description = 'Scrape Timago product details from discovered product URLs into one JSON dataset.';

    public function __construct(
        private readonly TimagoProductDataCrawler $crawler,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($explicitUrls !== []) {
            if (! $json) {
                $this->info('Scraping Timago product data from explicit product URLs...');
            }
            ...
            if (! $json) {
                $this->info('Scraping Timago product data from saved product-link discovery JSON...');
            }
        ...
        if ((bool) $this->option('show-failures') && $result['failed_urls'] !== []) {
            $this->newLine();
            $this->warn('Failed Timago product URLs:');
