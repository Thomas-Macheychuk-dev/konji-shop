...
final class ImportApoloniaProductsCommand extends Command
{
    protected $signature = 'apolonia:import
        {--from=scrapers/apolonia/full-product-data.json : Apolonia full product-data JSON path. Relative paths are resolved under storage/app.}
    ...
    protected $description = 'Import Apolonia scraped JSON into Konji Shop products.';

    public function __construct(
        private readonly ApoloniaProductImporter $importer,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($products === []) {
            $this->error('No Apolonia products found in data file: '.$dataPath);
        ...
        $this->info('Importing Apolonia products from: '.$dataPath);
        ...
        if ((bool) $this->option('show-failures') && $failures !== []) {
            $this->warn('Failed Apolonia imports:');
    ...
    {
        if (! is_file($path)) {
            $this->error('Apolonia product-data file not found: '.$path);
        ...
        try {
            ...
            $this->error('Apolonia product-data file is not valid JSON: '.$exception->getMessage());

            return [];
        }

        if (! is_array($decoded) || ! isset($decoded['products']) || ! is_array($decoded['products'])) {
            $this->error('Apolonia product-data file does not contain a products array: '.$path);
    ...
    private function resolvePath(string $path): string
    {
        ...
        if ($path === '') {
            $path = 'scrapers/apolonia/full-product-data.json';
        }
