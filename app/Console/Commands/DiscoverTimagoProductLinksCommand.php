...
final class DiscoverTimagoProductLinksCommand extends Command
{
    protected $signature = 'timago:product-links
        {--url=* : Timago category/menu page to scan. Defaults to https://www.timago.com.}
        {--categories-from= : Use an existing category discovery JSON file under storage/app instead of discovering categories again.}
        {--category-url=* : Explicit Timago category URL to scrape. When used, hierarchy discovery is skipped.}
        ...
        {--request-delay-ms=500 : Milliseconds to pause before each Timago HTTP request.}
        ...
        {--show-failures : Print failed Timago URLs.}';

    protected $description = 'Discover Timago product URLs from product-scraping categories.';

    public function __construct(
        private readonly TimagoProductUrlScraper $scraper,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($categoryUrls !== []) {
            if (! $json) {
                $this->info('Discovering Timago product URLs from explicit category URLs...');
            }
            ...
            if (! $json) {
                $this->info('Discovering Timago product URLs from saved category discovery JSON...');
            }
            ...
            if (! $json) {
                $this->info('Discovering Timago product URLs from category hierarchy...');
            }

            $urls = $this->option('url') ?: [TimagoCategoryUrlScraper::DEFAULT_URL];
