...
{
    protected $signature = 'relaxsan:import
        {--from=scrapers/relaxsan/full-product-data.json : RelaxSan full product-data JSON path. Relative paths are resolved under storage/app.}
    ...
    protected $description = 'Import RelaxSan scraped JSON into Konji Shop products.';

    public function __construct(
        private readonly RelaxSanProductImporter $importer,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($products === []) {
            $this->error('No RelaxSan products found in data file: '.$dataPath);
        ...
        $this->info('Importing RelaxSan products from: '.$dataPath);
        ...
        if ((bool) $this->option('show-failures') && $failures !== []) {
            $this->warn('Failed RelaxSan imports:');
    ...
    {
        if (! is_file($path)) {
            $this->error('RelaxSan product-data file not found: '.$path);
        ...
        try {
            ...
            $this->error('RelaxSan product-data file is not valid JSON: '.$exception->getMessage());

            return [];
        }

        if (! is_array($decoded) || ! isset($decoded['products']) || ! is_array($decoded['products'])) {
            $this->error('RelaxSan product-data file does not contain a products array: '.$path);
    ...
    private function resolvePath(string $path): string
    {
        ...
        if ($path === '') {
            $path = 'scrapers/relaxsan/full-product-data.json';
        }
