...
final class ImportVayaProductsCommand extends Command
{
    protected $signature = 'vaya:import
        {--from=scrapers/vaya/product-data.json : Vaya product-data JSON path. Relative paths are resolved under storage/app.}
        ...
        {--image-timeout=30 : Timeout in seconds for each Vaya image request.}
        {--image-attempts=5 : Maximum attempts for each Vaya image request.}
        {--image-retry-delay-ms=5000 : Milliseconds to wait between Vaya image retry attempts.}
        {--image-request-delay-ms=250 : Milliseconds to pause between Vaya image downloads.}
        {--show-failures : Print failed product imports at the end.}';

    protected $description = 'Import Vaya scraped JSON into Konji Shop products.';

    public function __construct(
        private readonly VayaProductImporter $importer,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($products === []) {
            $this->error('No Vaya products found in data file: '.$dataPath);
        ...
        $this->info('Importing Vaya products from: '.$dataPath);
    ...
    private function printDryRunSummary(array $products): void
    {
        ...
        $this->line('Distinct non-placeholder brands: '.count($brands));
    ...
    {
        if (! is_file($path)) {
            $this->error('Vaya product-data file not found: '.$path);
        ...
        try {
            ...
            $this->error('Vaya product-data file is not valid JSON: '.$exception->getMessage());

            return [];
        }

        if (! is_array($decoded) || ! isset($decoded['products']) || ! is_array($decoded['products'])) {
            $this->error('Vaya product-data file does not contain a products array: '.$path);
    ...
    private function resolvePath(string $path): string
    {
        ...
        if ($path === '') {
            $path = 'scrapers/vaya/product-data.json';
        }
    ...
    private function categoryPaths(array $product): array
    {
        ...
        foreach (($product['source_category_paths'] ?? []) as $path) {
            if (! is_array($path)) {
                continue;
            }

            $segments = array_values(array_filter(
                array_map(static fn (mixed $segment): string => is_string($segment) ? trim($segment) : '', $path),
        ...
        if ($paths === [] && is_array($product['source_category_path'] ?? null)) {
            $segments = array_values(array_filter(
                array_map(static fn (mixed $segment): string => is_string($segment) ? trim($segment) : '', $product['source_category_path']),
                static fn (string $segment): bool => $segment !== '',
            ));
    ...
    private function brandName(array $product): ?string
    {
        ...
        return in_array(mb_strtolower($name), ['', '-', '--', '—', 'brak', 'brak danych'], true)
            ? null
            : $name;
    }
}
