...
final class CrawlRehaFundProductDataCommand extends Command
{
    protected $signature = 'rehafund:crawl-product-data
        {--from=scrapers/rehafund/product-links.json : Product link discovery JSON file under storage/app.}
        {--url=* : Explicit RehaFund product URL to scrape instead of reading --from.}
        ...
        {--show-failures : Print failed RehaFund product URLs.}';

    protected $description = 'Scrape RehaFund product details from discovered product URLs into one JSON dataset.';

    public function __construct(
        private readonly RehaFundProductDataCrawler $crawler,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($explicitUrls !== []) {
            if (! $json) {
                $this->info('Scraping RehaFund product data from explicit product URLs...');
            }
            ...
            if (! $json) {
                $this->info('Scraping RehaFund product data from saved product-link discovery JSON...');
            }
        ...
        if ((bool) $this->option('show-failures') && $result['failed_urls'] !== []) {
            $this->newLine();
            $this->warn('Failed RehaFund product URLs:');
