...
use App\Services\Apolonia\ApoloniaProductDataCrawler;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\Storage;
use JsonException;

final class CrawlApoloniaProductDataCommand extends Command
{
    protected $signature = 'apolonia:crawl-product-data
        {--from=scrapers/apolonia/product-links.json : Product link discovery JSON file under storage/app.}
        {--url=* : Explicit Apolonia product URL to scrape instead of reading --from.}
        ...
        {--show-failures : Print failed Apolonia product URLs.}';

    protected $description = 'Scrape Apolonia product details from discovered product URLs into one JSON dataset.';

    public function __construct(
        private readonly ApoloniaProductDataCrawler $crawler,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($explicitUrls !== []) {
            if (! $json) {
                $this->info('Scraping Apolonia product data from explicit product URLs...');
            }
            ...
            if (! $json) {
                $this->info('Scraping Apolonia product data from saved product-link discovery JSON...');
            }
        ...
        if ((bool) $this->option('show-failures') && $result['failed_urls'] !== []) {
            $this->newLine();
            $this->warn('Failed Apolonia product URLs:');
