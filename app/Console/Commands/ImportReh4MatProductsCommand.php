...
use App\Services\Reh4Mat\Reh4MatProductImporter;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\Storage;
use JsonException;

final class ImportReh4MatProductsCommand extends Command
{
    protected $signature = 'reh4mat:import
        {--from=scrapers/reh4mat/full-product-data.json : Reh4Mat full product-data JSON path. Relative paths are resolved under storage/app.}
    ...
    protected $description = 'Import Reh4Mat scraped JSON into Konji Shop products.';

    public function __construct(
        private readonly Reh4MatProductImporter $importer,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($products === []) {
            $this->error('No Reh4Mat products found in data file: '.$dataPath);
        ...
        if ((bool) $this->option('show-failures') && $failures !== []) {
            $this->warn('Failed Reh4Mat imports:');
    ...
    private function printDryRunSummary(array $products): void
    {
        ...
        foreach ($products as $product) {
            ...
            $pictogramCount += is_array($product['pictograms'] ?? null) ? count($product['pictograms']) : 0;
        ...
        $this->line('Pictograms discovered: '.$pictogramCount);
        $this->line('Downloads discovered: '.$downloadCount);
    }
    ...
    {
        if (! is_file($path)) {
            $this->error('Reh4Mat product-data file not found: '.$path);
        ...
        try {
            ...
            $this->error('Reh4Mat product-data file is not valid JSON: '.$exception->getMessage());

            return [];
        }

        if (! is_array($decoded) || ! isset($decoded['products']) || ! is_array($decoded['products'])) {
            $this->error('Reh4Mat product-data file does not contain a products array: '.$path);
    ...
    private function resolvePath(string $path): string
    {
        ...
        if ($path === '') {
            $path = 'scrapers/reh4mat/full-product-data.json';
        }
