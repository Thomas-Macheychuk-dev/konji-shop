...
final class ImportZamstProductsCommand extends Command
{
    protected $signature = 'zamst:import-products
        {--from=scrapers/zamst/import-map.json : Zamst import-map JSON path. Relative paths resolve under storage/app.}
        {--write : Perform local database writes. Without this flag the command is a read-only dry-run.}
        {--allow-unverified-vat : Explicitly acknowledge mapped VAT review fallbacks for local draft imports.}
        ...
        {--image-limit=50 : Maximum images to import per product. Use 0 for no limit.}
        {--refresh-images : Re-download Zamst images even when the local file already exists.}
    ...
    protected $description = 'Import validated Zamst mapping into local Konji Shop draft products; read-only unless --write is supplied.';
    ...
    public function handle(): int
    {
        ...
        if ($products === []) {
            $this->error('No mapped Zamst products found in: '.$path);
        ...
        if (($map['ready_for_local_import_implementation'] ?? false) !== true || $hardErrors !== []) {
            $this->error('Zamst mapping is not structurally ready for local import.');
        ...
        $selectedVatReviewCount = count(array_filter(
            $selected,
            static fn (array $product): bool => ($product['tax']['requires_review'] ?? false) === true,
        ));

        $this->info('Zamst local product import');
        ...
        $this->line('Selected products requiring VAT review: '.$selectedVatReviewCount);
        ...
        if (! app()->environment('local', 'testing')) {
            $this->error('BLOCKED: zamst:import-products --write is allowed only in local/testing environments.');

            return self::FAILURE;
        }

        if ($selectedVatReviewCount > 0 && ! (bool) $this->option('allow-unverified-vat')) {
            $this->error('BLOCKED: selected Zamst products contain unverified VAT fallbacks.');
            $this->line('For this local draft-only import, re-run with --allow-unverified-vat to acknowledge the review state.');
        ...
        $this->newLine();
        $this->info('=== ZAMST LOCAL IMPORT RESULT ===');
        ...
        if ((bool) $this->option('show-failures') && $failures !== []) {
            $this->warn('Failed Zamst imports:');
        ...
        if ($failures === [] && (! $importImages || $stats['images_failed'] === 0)) {
            $this->info('PASS: selected Zamst products were imported locally as drafts.');
    ...
    private function printDatabaseAudit(array $productIds): void
    {
        ...
        $this->line('All local Zamst product rows: '.Product::query()->where('external_source', 'zamst')->count());
    }
    ...
    {
        if (! is_file($path)) {
            $this->error('Zamst import-map file not found: '.$path);
        ...
        try {
            ...
            $this->error('Zamst import-map is not valid JSON: '.$exception->getMessage());

            return null;
        }

        if (! is_array($decoded) || ($decoded['source'] ?? null) !== 'zamst') {
            $this->error('Zamst import-map has an invalid source marker.');
    ...
    private function resolvePath(string $path): string
    {
        ...
        if ($path === '') {
            $path = 'scrapers/zamst/import-map.json';
        }
    ...
    private function mappedProductName(array $mapped): string
    {
        ...
        return is_string($value) && trim($value) !== '' ? trim($value) : '[unnamed Zamst product]';
    }
