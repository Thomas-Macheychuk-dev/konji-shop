...
final class DiscoverAntarProductLinksCommand extends Command
{
    protected $signature = 'antar:product-links
        {--url=* : Antar category/menu page to scan. Defaults to https://antar.net/produkty/.}
        {--categories-from= : Use an existing category discovery JSON file under storage/app instead of discovering categories again.}
        {--category-url=* : Explicit Antar category URL to scrape. When used, hierarchy discovery is skipped.}
    ...
    protected $description = 'Discover Antar product URLs from WooCommerce product categories.';

    public function __construct(
        private readonly AntarProductUrlScraper $scraper,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($categoryUrls !== []) {
            if (! $json) {
                $this->info('Discovering Antar product URLs from explicit category URLs...');
            }
            ...
            if (! $json) {
                $this->info('Discovering Antar product URLs from saved category discovery JSON...');
            }
            ...
            if (! $json) {
                $this->info('Discovering Antar product URLs from category hierarchy...');
            }

            $urls = $this->option('url') ?: [AntarCategoryUrlScraper::DEFAULT_URL];
