...
final class ImportMedRehaProductsCommand extends Command
{
    protected $signature = 'medreha:import
        {--from=scrapers/medreha/full-product-data.json : MedReha full product-data JSON path. Relative paths are resolved under storage/app.}
    ...
    protected $description = 'Import MedReha scraped JSON into Konji Shop products.';

    public function __construct(
        private readonly MedRehaProductImporter $importer,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($products === []) {
            $this->error('No MedReha products found in data file: '.$dataPath);
        ...
        $this->info('Importing MedReha products from: '.$dataPath);
        ...
        if ((bool) $this->option('show-failures') && $failures !== []) {
            $this->warn('Failed MedReha imports:');
    ...
    {
        if (! is_file($path)) {
            $this->error('MedReha product-data file not found: '.$path);
        ...
        try {
            ...
            $this->error('MedReha product-data file is not valid JSON: '.$exception->getMessage());

            return [];
        }

        if (! is_array($decoded) || ! isset($decoded['products']) || ! is_array($decoded['products'])) {
            $this->error('MedReha product-data file does not contain a products array: '.$path);
    ...
    private function resolvePath(string $path): string
    {
        ...
        if ($path === '') {
            $path = 'scrapers/medreha/full-product-data.json';
        }
