...
use App\Services\TwojaPeruka\TwojaPerukaProductImporter;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\Storage;
use JsonException;

final class ImportTwojaPerukaProductsCommand extends Command
{
    protected $signature = 'twojaperuka:import
        {--from=scrapers/twojaperuka/full-product-data.json : TwojaPeruka full product-data JSON path. Relative paths are resolved under storage/app.}
    ...
    protected $description = 'Import TwojaPeruka scraped JSON into Konji Shop products.';

    public function __construct(
        private readonly TwojaPerukaProductImporter $importer,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($products === []) {
            $this->error('No TwojaPeruka products found in data file: '.$dataPath);
        ...
        $this->info('Importing TwojaPeruka products from: '.$dataPath);
        ...
        foreach ($selectedProducts as $index => $productData) {
            ...
            $name = is_string($productData['name'] ?? null) ? $productData['name'] : '[unnamed TwojaPeruka product]';
            ...
            try {
                $result = $this->importer->import($productData, $status, $importImages);
        ...
        if ((bool) $this->option('show-failures') && $failures !== []) {
            $this->warn('Failed TwojaPeruka imports:');
    ...
    {
        if (! is_file($path)) {
            $this->error('TwojaPeruka product-data file not found: '.$path);
        ...
        try {
            ...
            $this->error('TwojaPeruka product-data file is not valid JSON: '.$exception->getMessage());

            return [];
        }

        if (! is_array($decoded) || ! isset($decoded['products']) || ! is_array($decoded['products'])) {
            $this->error('TwojaPeruka product-data file does not contain a products array: '.$path);
    ...
    private function resolvePath(string $path): string
    {
        ...
        if ($path === '') {
            $path = 'scrapers/twojaperuka/full-product-data.json';
        }
