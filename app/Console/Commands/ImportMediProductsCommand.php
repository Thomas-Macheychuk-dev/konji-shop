...
final class ImportMediProductsCommand extends Command
{
    protected $signature = 'medi:import
        {--from=scrapers/medi/product-data.json : Medi full product-data JSON path. Relative paths are resolved under storage/app.}
    ...
    protected $description = 'Import Medi scraped JSON into Konji Shop products.';
    ...
    public function handle(): int
    {
        ...
        if ($products === []) {
            $this->error('No Medi products found in data file: '.$dataPath);
        ...
        $this->info('Importing Medi products from: '.$dataPath);
    ...
    private function printDryRunSummary(array $products): void
    {
        ...
        foreach ($products as $product) {
            ...
            $categoryCountBefore = count($categoryKeys);
            $this->addCategoryPathKeys($categoryKeys, $product['source_category_path'] ?? null);

            foreach (($product['source_category_contexts'] ?? []) as $context) {
                if (is_array($context)) {
                    $this->addCategoryPathKeys(
                        $categoryKeys,
            ...
            if (count($categoryKeys) === $categoryCountBefore) {
                $this->addCategoryPathKeys($categoryKeys, $product['categories'] ?? null);
            }

            if (count($categoryKeys) === $categoryCountBefore && is_string($product['category'] ?? null) && trim($product['category']) !== '') {
    ...
    /**
     * @param  array<string, true>  $categoryKeys
     */
    private function addCategoryPathKeys(array &$categoryKeys, mixed $value): void
    {
        if (! is_array($value)) {
            return;
        }
        ...
        foreach ($value as $categoryName) {
            ...
            $path[] = trim($categoryName);
            $categoryKeys[implode(' > ', $path)] = true;
        }
    }
    ...
    {
        if (! is_file($path)) {
            $this->error('Medi product-data file not found: '.$path);
        ...
        try {
            ...
            $this->error('Medi product-data file is not valid JSON: '.$exception->getMessage());

            return [];
        }

        if (! is_array($decoded) || ! isset($decoded['products']) || ! is_array($decoded['products'])) {
            $this->error('Medi product-data file does not contain a products array: '.$path);
    ...
    private function resolvePath(string $path): string
    {
        ...
        if ($path === '') {
            $path = 'scrapers/medi/product-data.json';
        }
