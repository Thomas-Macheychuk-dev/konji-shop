...
final class CrawlMediProductDataCommand extends Command
{
    protected $signature = 'medi:crawl-product-data
        {--from=scrapers/medi/product-links.json : Product link discovery JSON file under storage/app.}
        {--url=* : Explicit medi product URL to scrape instead of reading --from.}
        ...
        {--show-failures : Print failed medi product URLs.}';

    protected $description = 'Scrape Medi product details from discovered product URLs into one JSON dataset.';
    ...
    public function handle(): int
    {
        ...
        $this->crawler
            ...
            ->withRetry($this->attempts(), $this->retryDelayMilliseconds())
        ...
        if ($explicitUrls !== []) {
            if (! $json) {
                $this->info('Scraping medi product data from explicit product URLs...');
            }
            ...
            if (! $json) {
                $this->info('Scraping medi product data from saved product-link discovery JSON...');
            }
        ...
        if ((bool) $this->option('show-failures') && $result['failed_urls'] !== []) {
            $this->newLine();
            $this->warn('Failed medi product URLs:');
    ...
    private function printProductSummary(array $result): void
    {
        ...
        foreach ($result['products'] as $product) {
            ...
            $this->line('  Price: '.($product['price_gross_amount'] ?? 'not public').' '.($product['currency'] ?? 'PLN'));
            ...
            $this->line('  Available variants: '.($product['available_variant_count'] ?? 0));
        }
    }
    ...
    private function saveJson(string $relativePath, array $result, bool $quiet = false): void
    {
        ...
        if (! is_dir($directory) && ! mkdir($directory, 0755, true) && ! is_dir($directory)) {
            throw new RuntimeException('Unable to create Medi product-data directory: '.$directory);
        }
        ...
        if (file_put_contents($path, $json) === false) {
            throw new RuntimeException('Unable to save Medi product data JSON: '.$path);
        }
