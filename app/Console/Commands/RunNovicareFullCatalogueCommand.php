...
final class RunNovicareFullCatalogueCommand extends Command
{
    protected $signature = 'novicare:full-catalogue
        {--from=scrapers/novicare/product-links.json : Product-link discovery JSON path under storage/app.}
        {--runtime-dir=scrapers/novicare/runtime : Runtime manifest and product-data batch directory under storage/app.}
        {--refresh-discovery : Discover live Novicare categories and product links before starting a new runtime.}
        {--batch-size=5 : Number of Novicare product URLs to crawl and import per batch.}
        ...
        {--image-retry-delay-ms=5000 : Milliseconds to wait between Novicare image retries.}
        ...
        {--attempts=3 : Number of attempts per Novicare product request.}
        {--retry-delay-ms=1500 : Milliseconds to pause before retrying a failed Novicare product request.}
        {--request-delay-ms=500 : Milliseconds to pause before each Novicare product request.}
        {--insecure : Disable TLS certificate verification for Novicare crawl requests.}
    ...
    protected $description = 'Run the Novicare full catalogue crawl and import in resumable, logged batches.';

    public function __construct(
        private readonly NovicareProductDataCrawler $crawler,
        private readonly NovicareProductImporter $importer,
        private readonly NovicareProductUrlScraper $productUrlScraper,
    ) {
        parent::__construct();
    }
    ...
    {
        $sourceRelativePath = $this->relativePathOption('from', 'scrapers/novicare/product-links.json');
        $runtimeRelativePath = $this->relativePathOption('runtime-dir', 'scrapers/novicare/runtime');
        ...
        $source = $this->loadJson($sourcePath, 'Novicare product-link discovery JSON');
        $sourceContents = file_get_contents($sourcePath);

        if ($sourceContents === false) {
            throw new RuntimeException('Unable to read Novicare product-link discovery JSON: '.$sourcePath);
        }

        $productUrls = $this->productUrls($source);

        if ($productUrls === []) {
            $this->error('No Novicare product URLs were found in storage/app/'.$sourceRelativePath);
        ...
        if ($resume) {
            ...
            $manifest = $this->loadJson($manifestPath, 'Novicare runtime manifest');
            ...
            $this->error('A Novicare runtime manifest already exists. Use --resume or --reset.');
        ...
        $this->info('Running Novicare full catalogue runtime...');
        ...
        while ($nextOffset < count($productUrls)) {
            ...
            if ($reuseBatchData) {
                ...
                $productData = $this->loadJson($batchPath, 'Novicare product-data batch JSON');
            } else {
            ...
            if ($crawlFailures !== [] || $crawlStoppedEarly) {
                ...
                $batch['error'] = $crawlStoppedEarly
                    ? (string) ($productData['stop_reason'] ?? 'Novicare product crawl stopped early')
                    : 'One or more Novicare product URLs failed to crawl.';
                $manifest['batches'][$batchKey] = $batch;
        ...
        $this->newLine();
        $this->info('Novicare full catalogue runtime finished with status: '.$manifest['status']);
    ...
    {
        $this->info('Refreshing Novicare category and product-link discovery...');

        $result = $this->productUrlScraper
            ->withTlsVerification(! (bool) $this->option('insecure'))
            ->withTimeout($this->positiveIntOption('timeout', 20))
            ->withAttempts($this->positiveIntOption('attempts', 3))
            ->withRetryDelayMilliseconds($this->nonNegativeIntOption('retry-delay-ms', 1500))
        ...
        $this->line('Discovered categories: '.count($result['source_categories'] ?? []));
        $this->line('Discovered product URLs: '.count($result['product_urls'] ?? []));
        $this->line('Discovery failures: '.count($result['failed_urls'] ?? []));
        ...
        if (! $continueOnFailure) {
            throw new RuntimeException('Novicare product-link discovery reported failures. Retry or use --continue-on-failure.');
        }
    }
    ...
    private function newManifest(
        ...
        return [
            ...
            'log_path' => 'storage/logs/novicare/full-catalogue-'.$runId.'.jsonl',
    ...
    private function validateResumeManifest(
        ...
        if (($manifest['source'] ?? null) !== 'novicare') {
            throw new RuntimeException('Runtime manifest is not a Novicare manifest.');
        }

        if (($manifest['source_sha256'] ?? null) !== $sourceHash) {
            throw new RuntimeException('Novicare product-link discovery JSON changed since this runtime started. Use --reset to start a new runtime.');
        }
        ...
        if ((int) ($manifest['total_product_urls'] ?? -1) !== $totalProductUrls) {
            throw new RuntimeException('Novicare product URL count changed since this runtime started. Use --reset to start a new runtime.');
        }

        if (($manifest['execution_configuration'] ?? null) !== $executionConfiguration) {
            throw new RuntimeException('Novicare runtime mode, status, or image settings changed. Resume with the original settings or use --reset.');
        }
    }
    ...
    private function appendLog(array $manifest, string $event, array $context = []): void
    {
        ...
        if (! str_starts_with($logPath, 'storage/logs/')) {
            throw new RuntimeException('Invalid Novicare runtime log path.');
        }
        ...
        if (file_put_contents($path, $line.PHP_EOL, FILE_APPEND | LOCK_EX) === false) {
            throw new RuntimeException('Unable to append Novicare runtime operational log: '.$path);
        }
    }
    ...
    private function saveJson(string $path, array $data): void
    {
        ...
        if (file_put_contents($temporaryPath, $json, LOCK_EX) === false) {
            throw new RuntimeException('Unable to write temporary Novicare runtime JSON: '.$temporaryPath);
        }

        if (! rename($temporaryPath, $path)) {
            @unlink($temporaryPath);
            throw new RuntimeException('Unable to publish Novicare runtime JSON: '.$path);
        }
    }
    ...
    private function ensureDirectory(string $path): void
    {
        if (! is_dir($path) && ! mkdir($path, 0755, true) && ! is_dir($path)) {
            throw new RuntimeException('Unable to create Novicare runtime directory: '.$path);
        }
    }
    ...
    private function maybePrintFailures(array $manifest): void
    {
        ...
        foreach (($manifest['batches'] ?? []) as $batch) {
            ...
            foreach (($batch['crawl_failures'] ?? []) as $url => $reason) {
                if (! $printedHeading) {
                    $this->newLine();
                    $this->warn('Novicare runtime failures:');
                    $printedHeading = true;
                }
