...
final class DiscoverButterflyProductLinksCommand extends Command
{
    protected $signature = 'butterfly:product-links
        ...
        {--category-url=* : Explicit Butterfly category URL to scrape. When used, hierarchy discovery is skipped.}
    ...
    protected $description = 'Discover Butterfly product URLs from product-scraping categories.';

    public function __construct(
        private readonly ButterflyProductUrlScraper $scraper,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($categoryUrls !== []) {
            if (! $json) {
                $this->info('Discovering Butterfly product URLs from explicit category URLs...');
            }
            ...
            if (! $json) {
                $this->info('Discovering Butterfly product URLs from saved category discovery JSON...');
            }
            ...
            if (! $json) {
                $this->info('Discovering Butterfly product URLs from category hierarchy...');
            }
