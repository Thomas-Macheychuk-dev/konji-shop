...
final class DiscoverReh4MatProductLinksCommand extends Command
{
    protected $signature = 'reh4mat:product-links
        {--url=* : Reh4Mat catalogue/menu page to scan. Defaults to https://www.reh4mat.com/produkt/.}
        {--categories-from= : Use an existing category discovery JSON file under storage/app instead of discovering categories again.}
        {--category-url=* : Explicit Reh4Mat category URL to scrape. When used, hierarchy discovery is skipped.}
    ...
    protected $description = 'Discover Reh4Mat product URLs from allowed product-scraping categories.';

    public function __construct(
        private readonly Reh4MatProductUrlScraper $scraper,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($categoryUrls !== []) {
            if (! $json) {
                $this->info('Discovering Reh4Mat product URLs from explicit category URLs...');
            }
            ...
            if (! $json) {
                $this->info('Discovering Reh4Mat product URLs from saved category discovery JSON...');
            }
            ...
            if (! $json) {
                $this->info('Discovering Reh4Mat product URLs from allowed category hierarchy...');
            }
