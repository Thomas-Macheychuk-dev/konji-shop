...
use App\Services\RehaFund\RehaFundProductImporter;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\Storage;
use JsonException;

final class ImportRehaFundProductsCommand extends Command
{
    protected $signature = 'rehafund:import
        {--from=scrapers/rehafund/full-product-data.json : RehaFund full product-data JSON path. Relative paths are resolved under storage/app.}
    ...
    protected $description = 'Import RehaFund scraped JSON into Konji Shop products.';

    public function __construct(
        private readonly RehaFundProductImporter $importer,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($products === []) {
            $this->error('No RehaFund products found in data file: '.$dataPath);
        ...
        $this->info('Importing RehaFund products from: '.$dataPath);
        ...
        if ((bool) $this->option('show-failures') && $failures !== []) {
            $this->warn('Failed RehaFund imports:');
    ...
    {
        if (! is_file($path)) {
            $this->error('RehaFund product-data file not found: '.$path);
        ...
        try {
            ...
            $this->error('RehaFund product-data file is not valid JSON: '.$exception->getMessage());

            return [];
        }

        if (! is_array($decoded) || ! isset($decoded['products']) || ! is_array($decoded['products'])) {
            $this->error('RehaFund product-data file does not contain a products array: '.$path);
    ...
    private function resolvePath(string $path): string
    {
        ...
        if ($path === '') {
            $path = 'scrapers/rehafund/full-product-data.json';
        }
