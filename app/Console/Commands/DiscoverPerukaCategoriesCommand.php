...
final class DiscoverPerukaCategoriesCommand extends Command
{
    protected $signature = 'peruka:categories
        {--url=* : Peruka page to scan for category navigation. Defaults to https://www.peruka.pl/.}
    ...
    protected $description = 'Discover Peruka category URLs from the shop navigation.';

    public function __construct(
        private readonly PerukaCategoryUrlScraper $scraper,
    ) {
        parent::__construct();
    }
    ...
    {
        $urls = $this->option('url') ?: [PerukaCategoryUrlScraper::DEFAULT_URL];
        ...
        $this->info('Discovering Peruka category URLs...');
        ...
        if ((bool) $this->option('json')) {
            ...
            foreach ($result['categories'] as $category) {
                $prefix = $category['parent_name'] === null ? '' : $category['parent_name'].' > ';
                $this->line($prefix.$category['name'].' - '.$category['url']);
            }
        }
