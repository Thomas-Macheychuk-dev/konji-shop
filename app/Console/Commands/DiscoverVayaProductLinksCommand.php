...
final class DiscoverVayaProductLinksCommand extends Command
{
    protected $signature = 'vaya:product-links
        {--url=* : Vaya navigation page to scan. Defaults to https://www.vaya.com.pl/.}
        {--categories-from= : Use an existing Vaya category discovery JSON file under storage/app.}
        {--category-url=* : Explicit Vaya category URL to scrape. When used, hierarchy discovery is skipped.}
        {--page-limit= : Maximum number of paginated product-list pages to scrape per category.}
        {--category-limit= : Maximum number of product-scraping categories to scrape. Useful for smoke tests.}
        {--timeout=20 : HTTP request timeout in seconds.}
        {--attempts=3 : Maximum attempts per Vaya product-list request.}
    ...
    protected $description = 'Discover Vaya product URLs from the selected leaf category pages.';

    public function __construct(
        private readonly VayaProductUrlScraper $scraper,
    ) {
        parent::__construct();
    }
    ...
    public function handle(): int
    {
        ...
        if ($categoryUrls !== []) {
            if (! $json) {
                $this->info('Discovering Vaya product URLs from explicit category URLs...');
            }
            ...
            if (! $json) {
                $this->info('Discovering Vaya product URLs from saved category discovery JSON...');
            }
            ...
            if (! $json) {
                $this->info('Discovering Vaya product URLs from category hierarchy...');
            }
