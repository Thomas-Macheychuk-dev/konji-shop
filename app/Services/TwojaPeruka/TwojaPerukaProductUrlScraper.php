...
final class TwojaPerukaProductUrlScraper
{
    ...
    public function __construct(
        private readonly TwojaPerukaCategoryScraper $categoryScraper,
    ) {}
     ...
     */
    public function discover(array $startUrls = [TwojaPerukaCategoryScraper::DEFAULT_CATEGORY_URL]): array
    {
        return $this->scrape($startUrls)['product_urls'];
    }

    /**
     * Discover product links from the allowed TwojaPeruka category hierarchy.
     *
     * @param  array<int, string>  $startUrls
     * @return array{
     *     source: string,
    ...
    public function scrape(
        array $startUrls = [TwojaPerukaCategoryScraper::DEFAULT_CATEGORY_URL],
        ?int $pageLimit = null,
        ?int $categoryLimit = null,
    ): array {
        $this->emit('Discovering TwojaPeruka category hierarchy...');
    ...
    /**
     * Scrape product links from an existing TwojaPeruka category discovery JSON payload.
     *
    ...
    /**
     * Scrape product links from explicit category URLs without running category discovery first.
    ...
    private function extractProducts(string $html, string $baseUrl): array
    {
        ...
        $productAnchors = $xpath->query(
            '//product-tile//a[@href]'
            .' | //*[contains(concat(" ", normalize-space(@class), " "), " product-tile ")]//a[@href]'
            .' | //a[@href and contains(@aria-label, "Zobacz produkt")]'
        );

        if ($productAnchors === false || $productAnchors->length === 0) {
            return [];
        }

        $products = [];

        foreach ($productAnchors as $anchor) {
    ...
    {
        $ariaLabel = $this->normalizeLabel($anchor->getAttribute('aria-label'));

        if (preg_match('/^Zobacz produkt\s+(.+)$/u', $ariaLabel, $matches) === 1) {
    ...
    private function extractNextCategoryPageUrl(string $html, string $baseUrl): ?string
    {
        ...
        $queries = [
            ...
            '//a[@href and contains(@aria-label, "Następne produkty")]',
            '//a[@href and contains(concat(" ", normalize-space(@class), " "), " pagination__button ") and contains(normalize-space(.), "Następna")]',
        ];
    ...
    private function normalizeProductUrl(string $url, ?string $baseUrl = null): ?string
    {
        ...
        if ($path === '/' || $this->isNonProductPath($path)) {
            return null;
        }

        return 'https://'.self::TWOJAPERUKA_HOST.$path;
    }
    ...
    private function normalizeCategoryPageUrl(string $url, ?string $baseUrl = null): ?string
    {
        ...
        return 'https://'.self::TWOJAPERUKA_HOST.$this->normalizePath((string) parse_url($url, PHP_URL_PATH));
    }
