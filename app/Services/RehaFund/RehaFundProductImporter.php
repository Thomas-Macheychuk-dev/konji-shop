...
final class RehaFundProductImporter
{
     ...
     * @var list<string>
     */
    private const IMAGE_ALLOWED_HOSTS = ['sklep.rehafund.pl'];
        ...
        $baseSlug = $this->stringOrNull($scraped['slug'] ?? null)
            ?: $this->slugFromUrl($this->stringOrNull($scraped['canonical_url'] ?? null))
                ?: $this->slugFromUrl($this->stringOrNull($scraped['source_url'] ?? null))
                    ?: Str::slug((string) ($scraped['name'] ?? 'rehafund-product-'.$externalId));

        if ($baseSlug === '') {
            $baseSlug = 'rehafund-product-'.$externalId;
        }

        $name = $this->stringOrNull($scraped['name'] ?? null)
            ?: $this->stringOrNull($scraped['sku'] ?? null)
                ?: 'RehaFund product '.$externalId;
    ...
    private function resolveCategory(string $name, ?Category $parent, array $pathSegments): Category
    {
        ...
        if ($baseSlug === '') {
            $baseSlug = 'rehafund-category-'.substr(sha1(implode('|', $pathSegments)), 0, 10);
        }
    ...
    private function syncProductAttributes(Product $product, array $scraped): void
    {
        ...
        foreach (($scraped['attributes'] ?? []) as $attributeData) {
            ...
            if ($label === null || $value === null || $this->isGeneratedMetaLabel($label) || ! $this->isSafeFilterAttributeValue($value)) {
                continue;
            }
    ...
    private function syncVariants(Product $product, array $scraped, ProductStatus $status): void
    {
        ...
        foreach ($candidates as $index => $candidate) {
            $sourceExternalId = $this->stringOrNull($candidate['external_variant_id'] ?? null)
                ?: (string) ($index + 1);
            $externalVariantId = $this->limitDatabaseString('rehafund-'.$product->external_id.'-'.$sourceExternalId);
        ...
        foreach (($scraped['images'] ?? []) as $imageData) {
            ...
            try {
                $imported = $this->remoteImageImporter->import(
                    $url,
                    'products/rehafund/'.$product->external_id.'/gallery',
                    'public',
                    self::IMAGE_ALLOWED_HOSTS,
                );
            } catch (Throwable $exception) {
                $this->warnings[] = 'Image skipped for RehaFund product '.$product->external_id.': '.$url.' — '.$exception->getMessage();
    ...
    private function resolveAttributeValue(string $attributeName, string $value): AttributeValue
    {
        ...
        $value = trim($value);
        $limitedAttributeName = $this->limitDatabaseString($attributeName);
        $limitedValue = $this->limitDatabaseString($value);
        $attributeSlug = $this->databaseSlug($attributeName, 'rehafund-attribute');
        $valueSlug = $this->databaseSlug($value, 'rehafund-value');

        $attribute = Attribute::query()->firstOrCreate(
            ['slug' => $attributeSlug],
            [
                'name' => $limitedAttributeName,
    ...
    private function parentSku(array $scraped, string $externalId): string
    {
        ...
        return $this->limitDatabaseString($this->normaliseSku($sku) ?: 'REHAFUND-'.$externalId);
    }
    ...
    private function variantSku(array $scraped, array $candidate, string $externalId, ?string $label, string $sourceExternalId): string
    {
        ...
        return $sku ?: 'REHAFUND-'.$externalId.'-'.$sourceExternalId;
    }
    ...
    private function productDescriptionHtml(array $scraped, string $externalId, bool $localizeInlineImages): ?string
    {
        ...
        if ($this->booleanValue($scraped['is_medical_device'] ?? null) === true) {
            $sections[] = '<section class="rehafund-medical-notice"><p><strong>To jest wyrób medyczny. Używaj go zgodnie z instrukcją używania lub etykietą.</strong></p></section>';
        }
        ...
        if ($html === null || ! $localizeInlineImages) {
            return $html;
        }
    ...
    private function parameterSection(array $scraped): ?string
    {
        ...
        return '<section class="rehafund-parameters"><h2>Parametry produktu</h2><table><tbody>'.implode('', array_values(array_unique($rows))).'</tbody></table></section>';
    }
    ...
    private function variantSection(array $scraped): ?string
    {
        ...
        return '<section class="rehafund-variants"><h2>Dostępne warianty</h2><ul>'.implode('', array_values(array_unique($items))).'</ul></section>';
    }
    ...
    private function metadataSection(array $scraped): ?string
    {
        ...
        return '<section class="rehafund-product-meta"><h2>Dane produktu</h2><table><tbody>'.implode('', $rows).'</tbody></table></section>';
    }
    ...
    private function rewriteEmbeddedDescriptionImages(string $html, string $externalId): string
    {
        return preg_replace_callback('/<img\b[^>]*>/isu', function (array $matches) use ($externalId): string {
            $tag = $matches[0];
            $sourceUrl = $this->normalizeRehaFundImageUrl($this->attributeValue($tag, 'src'))
                ?: $this->normalizeRehaFundImageUrl($this->attributeValue($tag, 'data-src'))
                    ?: $this->normalizeRehaFundImageUrl($this->attributeValue($tag, 'data-lazy'));
            ...
            try {
                $imported = $this->remoteImageImporter->import(
                    $sourceUrl,
                    'products/rehafund/'.$externalId.'/description',
                    'public',
                    self::IMAGE_ALLOWED_HOSTS,
                );
                ...
                $this->warnings[] = 'Description image kept as remote URL for RehaFund product '.$externalId.': '.$sourceUrl.' — '.$exception->getMessage();
                $url = $sourceUrl;
            }
    ...
    private function normalizeRehaFundImageUrl(?string $url): ?string
    {
        if ($url === null) {
            return null;
        }
        ...
        if (str_starts_with($url, '//')) {
            ...
            $url = 'https://sklep.rehafund.pl'.$url;
        } elseif (! filter_var($url, FILTER_VALIDATE_URL)
            && preg_match('#^(?:img|usr|file)/#iu', $url) === 1) {
            $url = 'https://sklep.rehafund.pl/'.$url;
        }
    ...
    private function externalProductId(array $scraped): string
    {
        ...
        $sourceUrl = $this->stringOrNull($scraped['canonical_url'] ?? null)
            ?: $this->stringOrNull($scraped['source_url'] ?? null)
                ?: (string) ($scraped['name'] ?? 'rehafund-product');
    ...
    {
        $baseSlug = Str::slug($baseSlug) ?: 'rehafund-product-'.$externalId;
        $candidate = $baseSlug;
        $suffix = 2;

        while ($this->productSlugExists($candidate, $currentProductId)) {
            if ($suffix === 2) {
                $candidate = $baseSlug.'-rehafund-'.$externalId;
            } else {
                $candidate = $baseSlug.'-rehafund-'.$externalId.'-'.$suffix;
            }
    ...
    {
        $baseSlug = Str::slug($baseSlug) ?: 'rehafund-category';
        $candidate = $baseSlug;
        $suffix = 2;
    ...
    {
        $sku = $this->normaliseSku($sku) ?: 'REHAFUND-'.$externalVariantId;
        $candidate = $sku;
        $suffix = 2;
    ...
    private function databaseSlug(string $value, string $fallbackPrefix): string
    {
        $slug = Str::slug($value);

        if ($slug === '') {
            $slug = $fallbackPrefix.'-'.substr(sha1($value), 0, 10);
        }

        if (mb_strlen($slug) <= self::MAX_DATABASE_STRING_LENGTH) {
            return $slug;
        }

        $hash = substr(sha1($slug), 0, 10);
        $prefixLength = self::MAX_DATABASE_STRING_LENGTH - mb_strlen($hash) - 1;

        return rtrim(mb_substr($slug, 0, max(1, $prefixLength)), '-_').'_'.$hash;
    }
