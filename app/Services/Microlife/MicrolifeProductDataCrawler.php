...
final class MicrolifeProductDataCrawler
{
    ...
    public function __construct(
        private readonly MicrolifeProductScraper $productScraper,
    ) {}
    ...
    public function crawlProductUrls(
        ...
        foreach ($sourceUrls as $index => $sourceUrl) {
            ...
            if (($product['name'] ?? '') === '' || $productFailedUrls !== []) {
                ...
                if ($this->hasRateLimitFailure($productFailedUrls)) {
                    $stoppedEarly = true;
                    $stopReason = 'HTTP 429 rate limit or temporary block from Microlife';
        ...
        return [
            ...
            'consumer_product_count' => count(array_filter(
                $products,
                static fn (array $product): bool => ($product['catalogue_type'] ?? null) === 'consumer',
            )),
            'professional_product_count' => count(array_filter(
                $products,
                static fn (array $product): bool => ($product['catalogue_type'] ?? null) === 'professional',
            )),
            'products' => $products,
