...
final class NovicareProductDataCrawler
{
    ...
    public function __construct(
        private readonly NovicareProductScraper $productScraper,
    ) {}
        ...
        foreach ($sourceUrls as $index => $sourceUrl) {
            ...
            if (($product['name'] ?? '') === '' || $productFailedUrls !== []) {
                ...
                if ($this->hasRateLimitFailure($productFailedUrls)) {
                    $stoppedEarly = true;
                    $stopReason = 'HTTP 429 rate limit or temporary block from Novicare';
