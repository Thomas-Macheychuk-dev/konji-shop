...
final class VermeirenProductDataCrawler
{
    ...
    public function __construct(
        private readonly VermeirenProductScraper $productScraper,
    ) {}
        ...
        foreach ($sourceUrls as $index => $sourceUrl) {
            ...
            if (($product['name'] ?? '') === '' || $productFailedUrls !== []) {
                ...
                if ($this->hasRateLimitFailure($productFailedUrls)) {
                    $stoppedEarly = true;
                    $stopReason = 'HTTP 429 rate limit or temporary block from Vermeiren';
