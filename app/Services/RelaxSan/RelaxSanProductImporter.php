...
final class RelaxSanProductImporter
{
     ...
     * @var list<string>
     */
    private const IMAGE_ALLOWED_HOSTS = ['relaxsansklep.pl'];
        ...
        $baseSlug = $this->stringOrNull($scraped['slug'] ?? null)
            ?: $this->slugFromUrl($this->stringOrNull($scraped['canonical_url'] ?? null))
                ?: $this->slugFromUrl($this->stringOrNull($scraped['source_url'] ?? null))
                    ?: Str::slug((string) ($scraped['name'] ?? 'relaxsan-product-'.$externalId));

        if ($baseSlug === '') {
            $baseSlug = 'relaxsan-product-'.$externalId;
        }

        $name = $this->stringOrNull($scraped['name'] ?? null)
            ?: $this->stringOrNull($scraped['sku'] ?? null)
                ?: 'RelaxSan product '.$externalId;
    ...
    private function resolveCategory(string $name, ?Category $parent, array $pathSegments): Category
    {
        ...
        if ($baseSlug === '') {
            $baseSlug = 'relaxsan-category-'.substr(sha1(implode('|', $pathSegments)), 0, 10);
        }
    ...
    private function syncVariants(Product $product, array $scraped, ProductStatus $status): void
    {
        ...
        foreach ($candidates as $index => $candidate) {
            $sourceExternalId = $this->stringOrNull($candidate['external_variant_id'] ?? null)
                ?: (string) ($index + 1);
            $externalVariantId = $this->limitDatabaseString('relaxsan-'.$product->external_id.'-'.$sourceExternalId);
        ...
        foreach (($scraped['images'] ?? []) as $imageData) {
            ...
            try {
                $imported = $this->remoteImageImporter->import(
                    $url,
                    'products/relaxsan/'.$product->external_id.'/gallery',
                    'public',
                    self::IMAGE_ALLOWED_HOSTS,
                );
            } catch (Throwable $exception) {
                $this->warnings[] = 'Image skipped for RelaxSan product '.$product->external_id.': '.$url.' — '.$exception->getMessage();
    ...
    private function resolveAttributeValue(string $attributeName, string $value): AttributeValue
    {
        ...
        $value = trim($value);
        $attributeSlug = Str::slug($attributeName) ?: 'relaxsan-attribute-'.substr(sha1($attributeName), 0, 10);
        $valueSlug = Str::slug($value) ?: 'relaxsan-value-'.substr(sha1($value), 0, 10);
    ...
    private function parentSku(array $scraped, string $externalId): string
    {
        ...
        return $this->limitDatabaseString($this->normaliseSku($sku) ?: 'RELAXSAN-'.$externalId);
    }
    ...
    private function variantSku(array $scraped, array $candidate, string $externalId, ?string $label, string $sourceExternalId): string
    {
        ...
        return $sku ?: 'RELAXSAN-'.$externalId.'-'.$sourceExternalId;
    }
    ...
    private function productDescriptionHtml(array $scraped, string $externalId, bool $importDescriptionImages): ?string
    {
        ...
        if ($this->booleanValue($scraped['is_medical_device'] ?? null) === true) {
            $sections[] = '<section class="relaxsan-medical-notice"><p><strong>To jest wyrób medyczny. Używaj go zgodnie z instrukcją używania lub etykietą.</strong></p></section>';
        }
    ...
    private function parameterSection(array $scraped): ?string
    {
        ...
        return '<section class="relaxsan-parameters"><h2>Parametry produktu</h2><table><tbody>'.implode('', array_values(array_unique($rows))).'</tbody></table></section>';
    }
    ...
    private function variantSection(array $scraped): ?string
    {
        ...
        return '<section class="relaxsan-variants"><h2>Dostępne warianty</h2><ul>'.implode('', array_values(array_unique($items))).'</ul></section>';
    }
    ...
    private function metadataSection(array $scraped): ?string
    {
        ...
        return '<section class="relaxsan-product-meta"><h2>Dane produktu</h2><table><tbody>'.implode('', $rows).'</tbody></table></section>';
    }
    ...
    private function externalProductId(array $scraped): string
    {
        ...
        $sourceUrl = $this->stringOrNull($scraped['canonical_url'] ?? null)
            ?: $this->stringOrNull($scraped['source_url'] ?? null)
                ?: (string) ($scraped['name'] ?? 'relaxsan-product');
    ...
    {
        $baseSlug = Str::slug($baseSlug) ?: 'relaxsan-product-'.$externalId;
        $candidate = $baseSlug;
        $suffix = 2;

        while ($this->productSlugExists($candidate, $currentProductId)) {
            if ($suffix === 2) {
                $candidate = $baseSlug.'-relaxsan-'.$externalId;
            } else {
                $candidate = $baseSlug.'-relaxsan-'.$externalId.'-'.$suffix;
            }
    ...
    {
        $baseSlug = Str::slug($baseSlug) ?: 'relaxsan-category';
        $candidate = $baseSlug;
        $suffix = 2;
    ...
    {
        $sku = $this->limitDatabaseString($this->normaliseSku($sku) ?: 'RELAXSAN-'.$externalVariantId);
        $candidate = $sku;
        $suffix = 2;
