...
final class RelaxSanCategoryUrlScraper
{
    public const DEFAULT_URL = 'https://relaxsansklep.pl';

    private const RELAXSAN_HOST = 'relaxsansklep.pl';

    /**
     * Only catalogue roots relevant for Konji Shop are kept. RelaxSan also exposes
     * duplicate utility menu groups such as promotions, blog, contact and Asortyment.
     *
     * @var array<string, string>
     */
    private const ALLOWED_ROOT_CATEGORY_NAMES = [
        'przeciwzylakowe' => 'Przeciwżylakowe',
        'wyroby przeciwzylakowe' => 'Przeciwżylakowe',
        'przeciwzakrzepowe' => 'Przeciwzakrzepowe',
        'wyroby przeciwzakrzepowe' => 'Przeciwzakrzepowe',
        'bielizna' => 'Bielizna',
        'w ciazy' => 'W ciąży',
        'dla diabetykow' => 'Dla diabetyków',
        'ortopedyczne' => 'Ortopedyczne',
        'produkty ortopedyczne' => 'Ortopedyczne',
    ];
    ...
    /**
     * Discover RelaxSan category hierarchy from the Shoper menu.
     *
    ...
    public function scrape(array $startUrls = [self::DEFAULT_URL]): array
    {
        ...
        foreach ($startUrls as $startUrl) {
            ...
            $visited[$url] = true;
            $this->emit('Fetching RelaxSan category page: '.$url);
    ...
    private function extractRootCategoryTree(string $html, string $baseUrl): array
    {
        ...
        $rootMenus->each(function (Crawler $menu) use (&$roots, $baseUrl): void {
            ...
            foreach ($this->directChildElements($menuElement, 'li') as $menuItem) {
                $root = $this->rootCategoryFromListItem($menuItem, $baseUrl);
                ...
                // RelaxSan duplicates the category tree under a utility `Asortyment` menu.
                // Treat direct level1 children of non-category wrappers as possible root categories.
                ...
                foreach ($this->directChildElements($firstLevelList, 'li') as $rootItem) {
                    $nestedRoot = $this->rootCategoryFromListItem($rootItem, $baseUrl);
    ...
    private function isUtilityPath(string $path): bool
    {
        $path = mb_strtolower($path);

        return str_starts_with($path, '/pl/i/')
            || str_starts_with($path, '/pl/n/')
            || str_starts_with($path, '/pl/login')
            || str_starts_with($path, '/pl/reg')
            || str_starts_with($path, '/pl/basket')
            || str_starts_with($path, '/pl/panel')
            || str_starts_with($path, '/pl/promotions')
            || str_starts_with($path, '/faq')
            || str_starts_with($path, '/blog')
            || str_starts_with($path, '/kontakt')
            || str_starts_with($path, '/zwroty');
    }
    ...
    private function headers(): array
    {
        return [
            ...
            'User-Agent' => 'Mozilla/5.0 (compatible; KonjiShopRelaxSanScraper/1.0; +https://konji.pl)',
