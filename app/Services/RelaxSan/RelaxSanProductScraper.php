...
namespace App\Services\RelaxSan;
...
final class RelaxSanProductScraper
{
    private const RELAXSAN_HOST = 'relaxsansklep.pl';
    ...
    public function scrape(string $url, ?array $productLinkContext = null): array
    {
        ...
        $this->emit('Fetching RelaxSan product page: '.$sourceUrl);
        $html = $this->fetchBody($sourceUrl, $failed);

        if ($html === null) {
            $warnings[] = 'Unable to fetch RelaxSan product page.';
    ...
    public function extract(
        ...
        if ($sku !== null) {
            $this->addAttribute($attributes, 'Kod produktu', $sku);
        }

        if ($ean !== null) {
            $this->addAttribute($attributes, 'Kod EAN', $ean);
        }
    ...
    public function normalizeProductUrl(string $url, ?string $baseUrl = null): ?string
    {
        ...
        if ($url === null || ! $this->isRelaxSanUrl($url)) {
            return null;
        }
        ...
        foreach ([
            ...
            if ($pathLower === $blockedPath || str_starts_with($pathLower, $blockedPath.'/')) {
                return null;
            }
        }
        ...
        if (preg_match('#^/pl/c(?:/|$)#u', $path) === 1) {
            return null;
        }

        return 'https://'.self::RELAXSAN_HOST.$path;
    }
    ...
    private function extractName(Crawler $crawler, string $seoTitle): string
    {
        ...
        return $this->normalizeLabel(preg_replace('/\s*[-|]\s*RelaxSan\s*$/iu', '', $seoTitle) ?: $seoTitle);
    }
    ...
    private function extractDescriptionHtml(Crawler $crawler): ?string
    {
        foreach ([
            ...
            '#box_description .resetcss[itemprop="description"]',
    ...
    private function extractSku(Crawler $crawler, string $html, array $attributes): ?string
    {
        foreach ([
            ...
            try {
                $sku = $this->normalizeLabel($crawler->filter($selector)->first()->text(''));
            } catch (Throwable) {
                $sku = '';
            }

            if ($sku !== '' && ! str_contains(mb_strtolower($sku), 'kod produktu')) {
                return $sku;
            }
        }
        ...
        if (preg_match('/Kod produktu:\s*([^<\n\r]+)/iu', strip_tags($html), $matches) === 1) {
    ...
    private function extractImages(Crawler $crawler, string $baseUrl, string $name): array
    {
        ...
        foreach ([
            ...
            '#box_productfull .productimg a[href]' => 'href',
    ...
    private function extractVariantCandidates(Crawler $crawler, ?float $priceGrossAmount): array
    {
        ...
        foreach ([
            ...
            try {
                $crawler->filter($selector)->each(function (Crawler $select) use (&$groups, &$seenSelects): void {
                    $node = $select->getNode(0);

                    if (! $node instanceof DOMElement) {
                        return;
                    }

                    $selectKey = $this->variantSelectKey($node);

                    if (isset($seenSelects[$selectKey])) {
                        return;
                    }

                    $seenSelects[$selectKey] = true;

                    $label = $this->variantAttributeLabel($select);
                    $values = [];

                    $select->filter('option')->each(function (Crawler $option) use (&$values): void {
        ...
        foreach ($groups as $group) {
            ...
            foreach ($variants as $variant) {
                foreach ($group['values'] as $value) {
                    $next[] = array_merge($variant, [[
                        'group_label' => $group['label'],
                        'external_value_id' => $value['external_value_id'],
                        'label' => $value['label'],
                        'value' => $value['value'],
                    ]]);
                }
        ...
        return array_map(function (array $combination) use ($priceGrossAmount): array {
            $labels = array_map(
                fn (array $entry): string => $entry['group_label'].': '.$entry['value'],
                $combination,
            );
            $externalIds = array_map(
                fn (array $entry): string => (string) $entry['external_value_id'],
                $combination,
            );

            return [
                'external_variant_id' => implode('-', $externalIds),
                'sku' => null,
    ...
    private function variantSelectKey(DOMElement $select): string
    {
        $id = trim($select->getAttribute('id'));
        ...
        return 'node:'.spl_object_id($select);
    }
        ...
        if (is_string($id) && $id !== '') {
            ...
            if ($document instanceof DOMDocument) {
                ...
                if ($label instanceof DOMElement) {
                    $text = $this->normalizeAttributeLabel(preg_replace('/^\*\s*/u', '', $label->textContent ?? '') ?? '');
    ...
    private function normalizeUrl(string $url, ?string $baseUrl = null): ?string
    {
        ...
        if (str_starts_with($url, '//')) {
            ...
            $url = 'https://'.self::RELAXSAN_HOST.$url;
    ...
    private function normalizeAssetUrl(string $url, string $baseUrl): ?string
    {
        ...
        if ($normalized === null || ! $this->isRelaxSanUrl($normalized)) {
            return null;
        }
    ...
    private function normalizeHost(string $host): ?string
    {
        ...
        return match ($host) {
            self::RELAXSAN_HOST, 'www.'.self::RELAXSAN_HOST => self::RELAXSAN_HOST,
            default => null,
        };
    }

    private function isRelaxSanUrl(string $url): bool
    {
        return $this->normalizeHost((string) parse_url($url, PHP_URL_HOST)) === self::RELAXSAN_HOST;
    }
    ...
    private function normalizeAvailability(?string $availabilityLabel, string $html): string
    {
        ...
        if ($labelOnly !== '') {
            ...
            if ($this->containsAny($labelOnly, ['dostępny na zamówienie', 'dostepny na zamowienie', 'na zamówienie', 'na zamowienie'])) {
                return 'preorder';
            }

            if ($this->containsAny($labelOnly, ['duża ilość', 'duza ilosc', 'towar dostępny', 'towar dostepny', 'dostępny od ręki', 'dostepny od reki', 'dostępny', 'dostepny', 'wysyłka w', 'wysylka w', '24 godz', '48 godz'])) {
                return 'in_stock';
            }
        }
        ...
        if ($this->containsAny($pageText, ['dostępny na zamówienie', 'dostepny na zamowienie', 'na zamówienie', 'na zamowienie'])) {
            return 'preorder';
        }

        if ($this->containsAny($pageText, ['duża ilość', 'duza ilosc', 'towar dostępny', 'towar dostepny', 'dostępny od ręki', 'dostepny od reki', 'wysyłka w', 'wysylka w', '24 godz', '48 godz'])) {
            return 'in_stock';
        }
    ...
    private function isMedicalDevice(string $html, array $categories, array $attributes): bool
    {
        ...
        foreach (['wyrób medyczny', 'wyrob medyczny', 'sprzęt medyczny', 'sprzet medyczny', 'kompresja', 'kompresyjne', 'uciskowe', 'ucisku', 'przeciwżylakowe', 'przeciwzylakowe', 'przeciwzakrzepowe', 'orteza'] as $needle) {
            if (str_contains($haystack, $needle)) {
                return true;
            }
        }
    ...
    private function withProductLinkContext(array $product, ?array $context): array
    {
        ...
        return $product + [
            'source_category_name' => $this->nullableString($context['category_name'] ?? null),
            'source_category_url' => $this->nullableString($context['category_url'] ?? null),
            'source_top_category_name' => $this->nullableString($context['top_category_name'] ?? null),
            'source_top_category_url' => $this->nullableString($context['top_category_url'] ?? null),
            'source_category_path' => $this->stringList($context['category_path'] ?? []),
    ...
    {
        return $this->nullableString($context['category_name'] ?? null);
    }
