...
final class MedRehaProductScraper
{
    ...
    public function scrape(string $url, ?array $productLinkContext = null): array
    {
        ...
        $this->emit('Fetching MedReha product page: '.$sourceUrl);
        $html = $this->fetchBody($sourceUrl, $failed);

        if ($html === null) {
            $warnings[] = 'Unable to fetch MedReha product page.';
    ...
    private function extractName(Crawler $crawler, string $seoTitle): string
    {
        ...
        return $this->normalizeLabel(preg_replace('/\s*[-|]\s*MedReha\s*$/iu', '', $seoTitle) ?: $seoTitle);
    }
    ...
    private function normalizeAssetUrl(string $url, string $baseUrl): ?string
    {
        ...
        if ($normalized === null || ! $this->isMedRehaUrl($normalized)) {
            return null;
        }
    ...
    private function normalizeHost(string $host): ?string
    {
        ...
        return match ($host) {
            self::MEDREHA_HOST, 'www.'.self::MEDREHA_HOST => self::MEDREHA_HOST,
            default => null,
        };
    }
    ...
    private function isMedicalDevice(string $html, array $categories, array $attributes): bool
    {
        ...
        foreach (['wyrób medyczny', 'wyrob medyczny', 'sprzęt medyczny', 'sprzet medyczny', 'orteza', 'ciśnieniomierz', 'cisnieniomierz'] as $needle) {
            if (str_contains($haystack, $needle)) {
                return true;
            }
        }
