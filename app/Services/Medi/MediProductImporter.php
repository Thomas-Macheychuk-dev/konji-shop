...
final class MediProductImporter
{
    ...
    /** @var array<string, array{product_id: int, external_variant_id: string|null}> */
    ...
    public function import(
        ...
        $this->skuOwners = ProductVariant::withTrashed()
            ->whereNotNull('sku')
            ->get(['sku', 'product_id', 'external_variant_id'])
        ...
        $baseSlug = $this->stringOrNull($scraped['slug'] ?? null)
            ?: $this->slugFromUrl($this->stringOrNull($scraped['canonical_url'] ?? null))
                ?: $this->slugFromUrl($this->stringOrNull($scraped['source_url'] ?? null))
                    ?: Str::slug((string) ($scraped['name'] ?? 'medi-product-'.$externalId));

        if ($baseSlug === '') {
            $baseSlug = 'medi-product-'.$externalId;
        }

        $name = $this->stringOrNull($scraped['name'] ?? null)
            ?: $this->stringOrNull($scraped['sku'] ?? null)
                ?: 'Medi product '.$externalId;
    ...
    private function syncCategories(Product $product, array $scraped): void
    {
        ...
        foreach ($paths as $pathIndex => $categoryNames) {
            $parent = null;
            ...
            foreach ($categoryNames as $index => $categoryName) {
                $parent = $this->resolveCategory(
                    $categoryName,
                    $parent,
                    array_slice($categoryNames, 0, $index + 1),
                );
                $resolvedCategories[$parent->id] = $parent;
            }

            if ($pathIndex === 0 && $parent instanceof Category) {
                $primaryLeafId = $parent->id;
            }
        }
        ...
        foreach ($resolvedCategories as $category) {
            $syncPayload[$category->id] = [
                'is_primary' => $category->id === $primaryLeafId,
            ];
        }
    ...
    private function categoryPaths(array $scraped): array
    {
        ...
        $this->addCategoryPath($paths, $scraped['source_category_path'] ?? null);

        foreach (($scraped['source_category_contexts'] ?? []) as $context) {
            if (! is_array($context)) {
                continue;
            }
            ...
            $this->addCategoryPath(
                $paths,
                $context['source_category_path'] ?? $context['category_path'] ?? null,
            );
        }

        if ($paths === []) {
            $this->addCategoryPath($paths, $scraped['categories'] ?? null);
        }

        if ($paths === []) {
            $fallback = $this->stringOrNull($scraped['category'] ?? null)
        ...
        foreach ($paths as $index => $path) {
            if (count($path) > count($paths[$primaryPathIndex])) {
                $primaryPathIndex = $index;
            }
        }

        if ($primaryPathIndex > 0) {
            $primaryPath = $paths[$primaryPathIndex];
            unset($paths[$primaryPathIndex]);
            array_unshift($paths, $primaryPath);
        }
     ...
     */
    private function addCategoryPath(array &$paths, mixed $value): void
    {
        if (! is_array($value)) {
            return;
        }
        ...
        foreach ($paths as $existingPath) {
            if (implode(' > ', $existingPath) === $key) {
                return;
            }
        }
    ...
    private function resolveCategory(string $name, ?Category $parent, array $pathSegments): Category
    {
        ...
        if ($baseSlug === '') {
            $baseSlug = 'medi-category-'.substr(sha1(implode('|', $pathSegments)), 0, 10);
        }

        $category = Category::withTrashed()->where('slug', $baseSlug)->first();
        $expectedParentId = $parent?->id;

        if ($category !== null && $category->parent_id !== $expectedParentId) {
            $pathSlug = Str::slug(implode(' ', $pathSegments));

            if ($pathSlug === '' || $pathSlug === $baseSlug) {
                $pathSlug = $baseSlug.'-medi';
            }

            $category = Category::withTrashed()
                ->where('slug', $pathSlug)
                ->where('parent_id', $expectedParentId)
                ->first();
            $baseSlug = $pathSlug;
        }
    ...
    private function syncVariants(Product $product, array $scraped, ProductStatus $status): void
    {
        ...
        $defaultIndex = $this->defaultVariantIndex($candidates, $scraped);
        $vatRate = $this->vatRateForProduct($scraped);

        foreach ($candidates as $index => $candidate) {
            $sourceExternalId = $this->stringOrNull($candidate['external_variant_id'] ?? null)
                ?: (string) ($index + 1);
            $externalVariantId = $this->limitDatabaseString('medi-'.$product->external_id.'-'.$sourceExternalId);
            ...
            $variant = $existingVariants->get($externalVariantId);

            if ($variant instanceof ProductVariant && is_string($variant->sku) && $variant->sku !== '') {
                unset($this->skuOwners[$variant->sku]);
            }
     ...
     * @param  array<string, mixed>  $scraped
     */
    private function defaultVariantIndex(array $candidates, array $scraped): int
    {
        foreach ($candidates as $index => $candidate) {
            if ($this->stockStatus($candidate, $scraped) === StockStatus::IN_STOCK) {
                return $index;
            }
        }
    ...
    private function resolveAttributeValue(string $attributeName, string $value): AttributeValue
    {
        ...
        $value = trim($value);
        $attributeSlug = Str::slug($attributeName) ?: 'medi-attribute-'.substr(sha1($attributeName), 0, 10);
        $valueSlug = Str::slug($value) ?: 'medi-value-'.substr(sha1($value), 0, 10);

        $attribute = $this->attributeCache[$attributeSlug] ??= Attribute::query()->firstOrCreate(
            ['slug' => $attributeSlug],
            [
                'name' => $attributeName,
        ...
        $cacheKey = $attribute->id.'|'.$valueSlug;

        return $this->attributeValueCache[$cacheKey] ??= AttributeValue::query()->firstOrCreate(
            [
    ...
    private function variantCandidates(array $scraped): array
    {
        ...
        return [[
            ...
            'stock_quantity' => $scraped['stock_quantity'] ?? null,
        ]];
    }
    ...
    private function parentSku(array $scraped, string $externalId): string
    {
        ...
        return $this->limitDatabaseString($this->normaliseSku($sku) ?: 'MEDI-'.$externalId);
    }
    ...
    {
        $exactChildSku = $this->stringOrNull($candidate['sku'] ?? null);

        if ($exactChildSku !== null) {
            return $this->normaliseSku($exactChildSku);
        }

        $sku = $this->normaliseSku(
            $this->stringOrNull($scraped['sku'] ?? null) ?: 'MEDI-'.$externalId,
        );
        $suffix = $label !== null
            ? $this->normaliseSku($label)
            : ($sourceExternalId === 'default' ? '' : $this->normaliseSku($sourceExternalId));

        if ($suffix !== '' && ! Str::of($sku)->endsWith('-'.$suffix)) {
            $sku .= '-'.$suffix;
        }

        return $sku ?: 'MEDI-'.$externalId.'-'.$sourceExternalId;
    }
    ...
    private function productDescriptionHtml(array $scraped): ?string
    {
        ...
        if ($this->booleanValue($scraped['is_medical_device'] ?? null) === true) {
            $sections[] = '<section class="medi-medical-notice"><p><strong>To jest wyrób medyczny. Używaj go zgodnie z instrukcją używania lub etykietą.</strong></p></section>';
        }
    ...
    private function parameterSection(array $scraped): ?string
    {
        ...
        return '<section class="medi-parameters"><h2>Parametry produktu</h2><table><tbody>'.implode('', array_values(array_unique($rows))).'</tbody></table></section>';
    }
    ...
    private function metadataSection(array $scraped): ?string
    {
        ...
        return '<section class="medi-product-meta"><h2>Dane produktu</h2><table><tbody>'.implode('', $rows).'</tbody></table></section>';
    }
    ...
    {
        $availability = Str::of((string) ($candidate['availability'] ?? $scraped['availability'] ?? ''))
            ->lower()
            ->ascii()
            ->replaceMatches('/[^a-z0-9]+/', '_')
            ->trim('_')
            ->value();
        $quantity = $candidate['stock_quantity'] ?? null;
        ...
        if ($availability === 'in_stock' || (is_numeric($quantity) && (float) $quantity > 0)) {
        ...
        if ($label->contains(['brak', 'niedostepn', 'wyprzedan'])) {
            return StockStatus::OUT_OF_STOCK;
        }
    ...
    private function externalProductId(array $scraped): string
    {
        ...
        $sourceUrl = $this->stringOrNull($scraped['canonical_url'] ?? null)
            ?: $this->stringOrNull($scraped['source_url'] ?? null)
                ?: (string) ($scraped['name'] ?? 'medi-product');
    ...
    {
        $baseSlug = Str::slug($baseSlug) ?: 'medi-product-'.$externalId;
        $candidate = $baseSlug;
        $suffix = 2;

        while ($this->productSlugExists($candidate, $currentProductId)) {
            if ($suffix === 2) {
                $candidate = $baseSlug.'-medi-'.$externalId;
            } else {
                $candidate = $baseSlug.'-medi-'.$externalId.'-'.$suffix;
            }
    ...
    {
        $baseSlug = Str::slug($baseSlug) ?: 'medi-category';
        $candidate = $baseSlug;
        $suffix = 2;
    ...
    {
        $sku = $this->normaliseSku($sku) ?: 'MEDI-'.$externalVariantId;
        $candidate = $sku;
        $suffix = 2;

        while (isset($this->skuOwners[$candidate])) {
            $owner = $this->skuOwners[$candidate];

            if ($owner['product_id'] === $productId && $owner['external_variant_id'] === $externalVariantId) {
                break;
            }
