...
final class MediProductDataCrawler
{
    ...
    public function __construct(
        private readonly MediProductScraper $productScraper,
    ) {}
    ...
    public function crawlProductUrls(
        ...
        foreach ($sourceUrls as $index => $sourceUrl) {
            $this->emit('Scraping medi product '.($index + 1).'/'.$sourceUrlCount.': '.$sourceUrl);
            ...
            if (($product['name'] ?? '') === '' || $productFailedUrls !== []) {
                ...
                if ($this->hasRateLimitFailure($productFailedUrls)) {
                    $stoppedEarly = true;
                    $stopReason = 'HTTP 429 rate limit or temporary block from medi';
                    $this->emit('Stopping medi product crawl: '.$stopReason);
                    break;
                }
