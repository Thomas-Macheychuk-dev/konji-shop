...
final class ButterflyProductScraper
{
    ...
    public function scrape(string $url, ?array $productLinkContext = null): array
    {
        ...
        $this->emit('Fetching Butterfly product page: '.$sourceUrl);
        $html = $this->fetchBody($sourceUrl, $failed);

        if ($html === null) {
            $warnings[] = 'Unable to fetch Butterfly product page.';
    ...
    private function extractName(Crawler $crawler, string $seoTitle): string
    {
        ...
        return $this->normalizeLabel(preg_replace('/\s*[-|]\s*Butterfly\s*$/iu', '', $seoTitle) ?: $seoTitle);
    }
    ...
    private function normalizeAssetUrl(string $url, string $baseUrl): ?string
    {
        ...
        if ($normalized === null || ! $this->isButterflyUrl($normalized)) {
            return null;
        }
