        ...
        <div class="mb-8 flex items-start justify-between gap-4">
            <div>
                ...
                <p class="mt-2 text-sm text-zinc-600">
                    Manage products and package data used for Polkurier shipping valuation.
                </p>
            </div>
                ...
                <div class="grid gap-4 md:grid-cols-[1fr_auto] md:items-end">
                    <div>
                        ...
                        <input
                            ...
                            name="search"
                            value="{{ $search }}"
                            placeholder="Search by product name, slug, external ID, external SKU or variant SKU..."
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        <label class="flex items-center gap-2 rounded-xl border border-zinc-200 bg-white px-4 py-3 text-sm text-zinc-700">
                            ...
                            Missing weight/dimensions only
                        </label>
            ...
            <div class="overflow-hidden rounded-xl border border-zinc-200">
                <table class="min-w-full divide-y divide-zinc-200">
                    ...
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-zinc-500">Variants</th>
                        <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-zinc-500">Package data</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500">External source</th>
                        <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-zinc-500">Action</th>
                    </tr>
                    </thead>
                            ...
                            <td class="px-4 py-4 text-sm">
                                ...
                                @if ($product->external_parent_sku)
                                    <div class="mt-1 text-xs text-zinc-400">
                                        Parent SKU: {{ $product->external_parent_sku }}
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-4 text-right text-sm">
                                @if ((int) $product->variants_missing_package_data_count > 0)
                                    <span class="inline-flex rounded-full bg-red-100 px-2.5 py-1 text-xs font-semibold text-red-700">
                                        {{ $product->variants_missing_package_data_count }} missing weight/dimensions
                                    </span>
                                @else
